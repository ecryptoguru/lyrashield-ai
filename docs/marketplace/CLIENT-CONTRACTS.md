...
- Public npm metadata reports `@lyrashield/mcp@0.2.10` published. Its tarball matches the verified main artifact's SHA-512 receipt, contains the stdio entrypoint and answers MCP initialization plus `tools/list` with 21 tools using a synthetic credential and local-only API URL. Deliberate release updates must update generator, registry, templates, validator and snapshots together.
