...
| B5-S55 | OWASP Autonomous Penetration Testing Standard, Reporting                     | Living project standard; accessed 2026-07-17        | https://owasp.github.io/APTS/standard/8_Reporting/                                                                                                                        | Evidence provenance, coverage disclosure, finding states, limitations, and human review                |
...
| B5-P02 | The repository contains a GitHub Actions diff gate that verifies immutable base and head commits, checks changed files, runs secret and dependency checks, generates SARIF 2.1.0, optionally uploads SARIF, and makes a separate severity decision. Its local modes are `SAFE` and `AGGRESSIVE`; `DEEP` is rejected with directions to the hosted scan.                                                                                                                       | `.github/workflows/lyrashield-scan.yml`                                                                                               | Describe repository code, not a hosted public service. Do not claim that the local workflow performs hosted `DEEP` analysis.                         |
...
| B5-P04 | STALE — superseded by the versioned launch gate, which returns `READY`, `NOT_READY` or `INSUFFICIENT_EVIDENCE`; inconclusive renders neutral and never green. Do not reuse the old `GO`/`GO_WITH_CONDITIONS`/`NO_GO` framing in published copy. (Recorded 2026-07-17: launch readiness returned `NOT_EVALUATED`, `GO`, `GO_WITH_CONDITIONS` or `NO_GO` with a null score when no completed scan existed; thresholds were LyraShield product logic, not an industry standard.) | `apps/web/src/lib/launch-readiness.ts`, `apps/web/src/lib/launch-readiness.test.ts`                                                   | Never present the score or gate result as a warranty, compliance result, or universal release policy.                                                |
...
| B5-P06 | New reports freeze creation-time snapshots and include findings, retest summary, methodology, and limitations.                                                                                                                                                                                                                                                                                                                                                                | `packages/db/src/report-generator.ts`, `packages/db/src/report-generator.test.ts`                                                     | Do not claim customers, production assessments, benchmarks, or a complete engagement without retained evidence.                                      |
...
| LyraShield's repository implementation resolves exact commits, reports findings separately, fails required scanner infrastructure errors, and then applies a threshold decision. | B5-P02         | State that local modes are `SAFE` and `AGGRESSIVE`; hosted `DEEP` is rejected by the local workflow. |
...
| STALE — LyraShield's launch gate now returns READY, NOT_READY or INSUFFICIENT_EVIDENCE and the old four-verdict framing is retired. | B5-P04         | Do not reuse the retired framing in published copy or export those thresholds as industry guidance. |
