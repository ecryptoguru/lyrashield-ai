...
Local verification after remediation: 36/36 typecheck tasks, 34/34 lint tasks, 3,545 passing tests, 11/11 build tasks, formatting, migration drift, and `git diff --check`. All 81 migrations were also applied to an isolated database and exercised with a restricted role; null-context and cross-workspace access failed closed. These are local results for implementation commit `e861fda966b008cb47035c267aa8f6a4fb08b3f6`. PR #630 exact-head CI, merge, deployment, release, and native lifecycle acceptance remain separate gates tracked in the evidence ledger.
