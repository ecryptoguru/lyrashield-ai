...
| Public API `/api/v1` | Additive-only: no removed fields, renamed paths, changed status semantics or narrowed request shapes; breaking changes ship as `/api/v2`; ≥90-day deprecation notice with OpenAPI `deprecated: true` and a documented migration path (full policy: [`policies.md`](./policies.md)) |
