...
`LYRASHIELD_WEB_SEARCH_ENABLED` defaults to `1` in `ops/worker/run-worker.sh` but requires the API key to be present in `worker.env` to do useful work. Other settings (`MODE`, `MAX_RESULTS`, `MAX_CHARS_TOTAL`, `MAX_CALLS_PER_SCAN`, `BUDGET_USD`) may be overridden in `worker-runtime.conf` or the deployment shell; see `.env.example` for the full set. The engine redacts target hostnames, secrets, and PII from the query and tracks web-search cost separately from the LLM budget. The five-minute egress refresh applies new firewall rules immediately but defers the worker restart while `/tmp/lyrashield-worker-active` exists, then performs the pending restart on the first idle timer tick. This prevents DNS pin rotation from interrupting provider-billable scans.
