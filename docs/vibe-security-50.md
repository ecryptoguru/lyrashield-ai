...
Unit and contract tests prove registry completeness, instruction delivery, evidence separation, URL signatures, dependency parsing, agent-instruction detection, CI confused-deputy detection, and orchestration. Production coverage still requires the controlled scan gate with authorized models, a pinned sandbox image, an approved target, retained events, and the deployment-level egress control documented in `PRD.md`.
