...
- Worker image `sha256:8b76634439219d21f847be816f98b823f9e86e94f1128126df85608aa8dd36be` combines application commit `f03d891` with engine commit `16273ee`; the sandbox remains separately digest-pinned. ACR admin credentials are disabled after managed-identity pull was confirmed.
...
- Three approved Safe scans ended `FAILED / ENGINE_ERROR` after 8m11s, 8m05s, and 8m10s with zero findings and no assurance result. They are retained as failure evidence, not promoted as scan proof, and none was automatically replayed. Luna and Terra minimal provider probes passed. The second run exposed a diagnostics bug: a 14.7 KiB traceback displaced the fixed class marker. App PR #135 retains that marker while streaming and engine PR #15 suppresses target-derived non-interactive tracebacks. The third run then retained `BadRequestError`; Azure recorded one initial Luna `createChatCompletion` 400 for each attempt and no scan token processing. Engine PR #16 adds only an allowlisted provider code/type/param to the class marker so the request rejection can be corrected without retaining message text.
