...
- Production admin preflight `32728732596` failed safely before account reads or mutations because its workflow omitted Prisma generation; PR #413 fixed that. Retry `32729923445` then exposed the standalone DB client's explicit production-env requirement, again before account reads or mutations. The follow-up workflow fix supplies the existing database, Better Auth, and canonical URL values; production account state remains unproven until preflight is repeated.
