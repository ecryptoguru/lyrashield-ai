...
The account-owned Cloudflare token is stored as `CLOUDFLARE_API_TOKEN` with only Workers Scripts Write, Workers KV Storage Write, D1 Write, Account Settings Read, and Workers Routes Write. It expires on July 16, 2027.

The first automated main-branch release completed in CI run `29454682106` and deployed Worker version `eba63368-9dd4-4dcb-bb0c-09f46c26ec7f`. Independent production verification passed the canonical and path/query-preserving `www` redirect; API validation, CSRF, not-found, body-limit, and content-type boundaries; CSP/HSTS/no-store/noindex headers; 10 sitemap pages and 23 discovered internal URLs with zero broken links or metadata/schema issues; and a rendered Brave inspection. Live homepage Lighthouse reported zero console errors and scored 91/100/100/100 mobile and 94/100/100/100 desktop for performance/accessibility/best-practices/SEO, with zero total blocking time. Real-user Core Web Vitals and search/AI discovery remain measurement work rather than deployment proof.
