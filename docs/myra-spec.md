...
| Provider/model routing | Platform-owned, two-tier: simple queries → `gpt-5.4-nano` class, complex → `gpt-5.6-luna` class. Model names are env-configurable (`MYRA_MODEL_FAST`/`MYRA_MODEL_DEEP`); exact provider IDs verified at production enablement. Standard API privacy terms accepted | Before production model calls |
