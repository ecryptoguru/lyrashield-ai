...
| Provider/model routing | Platform-owned, single-model: every generated turn uses the platform's `gpt-6-luna` deployment, configured with `MYRA_MODEL`. Exact provider IDs verified at production enablement. Standard API privacy terms accepted                                                                                                                                                    | Before production model calls       |
