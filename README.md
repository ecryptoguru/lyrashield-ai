...
- `packages/agent-plugin` — the portable Agent Plugins v1.0.0 package (now v0.1.26 with Cursor streamable-http support) that bundles the MCP server and a `lyrashield` skill for the five preferred Agent Plugin clients (Claude Code, Cursor, OpenAI Codex, GitHub Copilot, Kiro). GitHub Copilot remains experimental until a retained client-runtime receipt exists.
