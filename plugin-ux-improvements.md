...
Make the LyraShield agent plugin truly one-command: auto-detect the current project, remember it as the default, switch projects easily, and have the agent skill guide the user through the right review depth with clear examples.

## Decisions (confirmed)

- Accounting stays private. The plugin should make mode choice clear and recommend the least intensive mode that answers the question.
...
- [x] **Task 1 — SKILL.md rewrite**
  - Add a review-depth table mapping user intent to goal + mode.
