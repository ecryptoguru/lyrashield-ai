...
- **Deep/Custom scans ARE available in Local mode.** The "Deep = Pro+" gate is a **Cloud** tier control (it bounds *our* Terra COGS). In Local, the customer's own AI pays for the deeper scan, so gating it would be artificial. **Do not port Cloud's scan-depth gating into Local.** (License tier may still gate *seats/machines*, not scan depth.)
...
- **Perpetual fallback logic:** after the 1-year update window, the app keeps running the **last build it was eligible for** indefinitely; it just stops accepting *newer* updates. Never deactivates.
