...
grep -Fq 'validate-manual-production-dispatch:' "$production_workflow"
...
grep -Fq 'retry_app_update() {' <<< "$mtls_patch_block"
...
candidate_probe_line=$(grep -n '"https://${CANDIDATE_FQDN}${path}"' "$production_workflow" | cut -d: -f1)
...
grep -Fq 'current-main: ${{ steps.current-main.outputs.current }}' "$repo/.github/workflows/release-production.yml"
grep -Fq 'Confirm release source is current main' "$repo/.github/workflows/release-production.yml"
grep -Fq 'Skipping superseded release ${HEAD_SHA}; main is now ${latest_main}.' "$repo/.github/workflows/release-production.yml"
grep -Fq "needs.routing.outputs.current-main == 'true'" "$repo/.github/workflows/release-production.yml"
