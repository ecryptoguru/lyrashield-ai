...
test("agent onboarding has OAuth-first setup and an explicit approval boundary", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /Set up LyraShield for your agent/i })).toHaveAttribute(
    "href",
...
test("mobile navigation reaches agent onboarding", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Release assurance your coding agent can act on/i })).toBeVisible()
})
