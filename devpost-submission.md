...
We added a shared, deterministic WebMCP assurance layer to LyraShield AI. It discovers imperative and declarative tool definitions, evaluates ten versioned controls, keeps incomplete analysis explicitly inconclusive, and supports only safe, deterministic rewrites. The same normalized model powers the free local Security Lab, repository scanning, CLI/SARIF, a GitHub Action subset, and report receipts.
...
- Deterministic inventory and ten controls for behavior/annotation mismatches, external-content hints, exposure, origin isolation, confirmation, input/output bounds, cancellation, lifecycle cleanup, runtime validation, and ambiguous contracts.
- A fail-closed rewrite preview that currently automates only statically provable wildcard exposure to same-origin scope; all other cases stay review-required.
...
3. Run `analyze_webmcp_source` with `{ "source": "unsafe_sample" }`. Expect a bounded result and a completed visible activity receipt.
4. Run `prepare_webmcp_rewrite` with `{ "controlId": "WEBMCP-03" }`. Expect `applyRequiredHumanReview: true` and no raw source or diff in the agent output.
...
TODO — publish a public YouTube video under three minutes with audio. Required sequence: problem (0:00–0:15), native WebMCP public-lab call (0:15–1:10), visible human rewrite/Undo (1:10–1:40), CLI/CI/report evidence (1:40–2:20), dashboard confirmation/receipt (2:20–2:50), and limitations/closing (2:50–3:00).
