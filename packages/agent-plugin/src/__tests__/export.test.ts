...
describe("exportMarketplace", () => {
  ...
  it("exports only the public client boundary with provenance", async () => {
    ...
    expect(manifest.generator).toEqual({ package: "@lyrashield/agent-plugin", version: "0.1.27" })
    ...
    ).resolves.toMatch(/id: "lyrashield-review"[\s\S]*version: "0\.1\.27"[\s\S]*mcpServers:/)
    await expect(
...
describe("exported validator", () => {
  it.each([
    ...
    ["gemini-extension.json", "@lyrashield/mcp@0.2.8", "@lyrashield/mcp"],
