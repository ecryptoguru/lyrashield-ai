 ...
 * emitted while it is in flight. Deliberately a module-level variable: the
 * logger is a shared primitive also bundled into the Next.js edge runtime, so
 * node:async_hooks (AsyncLocalStorage) is not available here. Single-variable
 * assignment per request is safe for logging (unlike tenant context, a swapped
 * id cannot leak data across tenants) and keeps this module runtime-agnostic.
 *
