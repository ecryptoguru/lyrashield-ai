...
- The catalog reserves `agent-plugin` install strategy entries for 6 clients (Claude Code, Cursor, VS Code, OpenAI Codex, GitHub Copilot, Kiro). The canonical `@lyrashield/agent-plugin` package currently emits manifest shims for the 4 clients with confirmed support today (Claude Code, Cursor, OpenAI Codex, Kiro); the VS Code and GitHub Copilot entries remain reserved.
...
- `getAgent(id)`, `listAgents()`, `agentsByStrategy(strategy)`, `AGENTS` — `agentsByStrategy("agent-plugin")` returns the 6 launch-client entries (Claude Code, Cursor, VS Code, OpenAI Codex, GitHub Copilot, Kiro); the canonical `@lyrashield/agent-plugin` package currently builds shims for the 4 with confirmed support today (Claude Code, Cursor, OpenAI Codex, Kiro).
