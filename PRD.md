...
Offline proof: 597 tests (1 skipped), Ruff, formatting, headless mypy, Bandit, package/native-binary checks, sandbox smoke, and worker compatibility
...
Current worker policy: Safe/Quick use GPT-5.6 Luna at medium reasoning with a $1.20 default cap; Standard uses Luna at medium with $3.20. Deep/Custom use a GPT-5.6 Terra/medium root coordinator and Luna/medium child specialists with a $15 default cap. A finite positive `Policy.maxBudgetUsd` overrides the mode default. `LYRASHIELD_LLM` is the fallback if a selected deployment variable is unset. Child work is deterministically assigned to Luna; the model cannot self-promote specialists to Terra, and only the root can create or stop specialists.
...
- Current data surface: **43 Prisma models**, **21 enums**, and **31 committed migrations**. Postgres RLS covers all 30 tables: the 21 direct workspace-scoped tables plus the 9 child tables (`ScanEvent`, `Evidence`, `ScanResultManifest`, `ScanCoverageReceipt`, `FixProposal`, `PullRequest`, `Ticket`, `ScorecardShare`, `ScorecardEvent`). The child tables had policies defined by `20260803000001_child_table_rls`, enabled by `20260803000002_child_table_rls_enable`, disabled by `20260803000003_child_table_rls_disable` after a production `42501` outage, and **re-enabled by `20260807000003_child_table_rls_re_enable` on 2026-08-07** once the cause was traced to a single write path outside `withWorkspaceRLS` (Deep Review v12 P0-1). The manifest and coverage receipts are intentionally child-scoped through `Scan`. Migration `20260808000000_oauth_provider` adds `OAuthClient`, `OAuthConsent`, and `OAuthToken` models for hosted MCP OAuth.
...
3. **Production infrastructure:** provision production PostgreSQL, a BullMQ-compatible TLS Redis endpoint (REST-only Upstash credentials do not operate the queue), mandatory private S3-compatible evidence storage, secrets, TLS, backups, monitoring, dedicated worker capacity with the engine and pinned sandbox, and the authenticated Next.js application origin. Apply and verify all 30 migrations on a fresh database, including scorecard events, single-use approvals, evidence idempotency, report snapshots, result-integrity receipts, scan-cost accounting, the public-score lookup index, global provider installation uniqueness, canonical GitHub installation IDs, active-scorecard uniqueness, finding statusReason, the scan/workspace/status index, the UX V2 schema (trust plans, notification preferences, scan duration), and fail-closed RLS for nine child tables. Reconcile legacy duplicate provider bindings before the uniqueness migration; evidence persistence fails closed until the configured `S3_*` endpoint succeeds.
