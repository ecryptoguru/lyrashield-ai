...
- Merged PR #115 baseline plus follow-up: lint, typecheck, production build, formatting, Prisma client generation, migration drift/application, SCA/secret scanning, security diff gate, CodeRabbit, `git diff --check`, **1013 core tests in 115 files**, **80 marketing tests in 12 files**, **16 motion tests**, and **4 passing Playwright Chromium tests**. Historical checkpoint counts below remain dated evidence, not the current release gate.
- Current product surface: **25 page route files** and **44 API route files** in `apps/web`.
- Current data surface: **39 Prisma models**, **18 enums**, and **26 committed migrations**. Postgres RLS covers 20 direct workspace-scoped tables; the manifest and coverage receipts are intentionally child-scoped through `Scan`.
...
- Current merged-engine proof: engine PRs #6, #7, and #20 are on `main`; the coordinated gate passes 329 tests, Ruff, formatting, headless mypy, Bandit, native-binary checks, sandbox build/smoke, and worker compatibility.
