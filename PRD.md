...
- Prisma migration `20260803000001_child_table_rls` defined fail-closed RLS policies for `ScanEvent`, `Evidence`, `ScorecardShare`, `ScorecardEvent`, `ReferralCode`, `ReferralAttribution`, `NotificationPreference`, and `OnboardingState`, and `20260803000002_child_table_rls_enable` enabled them. **`20260803000003_child_table_rls_disable` subsequently disabled RLS on these nine child tables after a production `42501` outage.** Database-level RLS coverage is currently 21 workspace-scoped tables, not 29. The nine child tables rely on application-layer enforcement (`withWorkspaceRLS` + service-layer checks) pending diagnosis and re-enable; see Deep Review v12 P0-1.
...
- Current data surface: **40 Prisma models**, **18 enums**, and **30 committed migrations**. Postgres RLS covers 21 direct workspace-scoped tables. The 9 child tables (`ScanEvent`, `Evidence`, `ScanResultManifest`, `ScanCoverageReceipt`, `FixProposal`, `PullRequest`, `Ticket`, `ScorecardShare`, `ScorecardEvent`) had RLS policies defined by `20260803000001_child_table_rls` and enabled by `20260803000002_child_table_rls_enable`, but `20260803000003_child_table_rls_disable` **disabled RLS on them after a production `42501` outage**. These tables currently rely on application-layer enforcement only; re-enable is pending diagnosis (Deep Review v12 P0-1). The manifest and coverage receipts are intentionally child-scoped through `Scan`.
