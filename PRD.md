...
>
> ⚠️ **2026-07-11 THIN-FORK RELEASE-GATE UPDATE:** The engine now has a weekly/manual, review-only upstream-sync workflow: it verifies ancestry, aborts rewritten history with exit `20`, rebases and verifies only when needed, and opens an `automation/upstream-<short-sha>` PR—never auto-merging, force-pushing, or resolving conflicts. Baseline is `7b639505fecf20a2d9e356f96bd91470aa828182`; the adapter maps product variables only when an explicit upstream `STRIX_*` value is absent and telemetry defaults off. Worker compatibility covers both `strix_runs` and `lyrashield_runs`. Current offline proof is 155 engine tests plus Ruff/format/mypy/Bandit and 600 application tests plus lint/typecheck/build. The worker Docker build now completes after scoping Next.js compilation to `@lyrashield/web`; local image ID `sha256:71d6c104f5d11e30d8f8ee63cef8aacb1819b5ec8a4c3d1987d7fd3dcaddc4e6` passed `lyrashield --version` (`1.0.4.post1`) and missing-model validation before any sandbox pull. No sandbox execution is claimed; production needs a pinned sandbox image digest and authorized LLM configuration before a controlled scan. No writable `origin` exists for the engine fork, and no controlled scan was run. See `codebase.md` §30.
