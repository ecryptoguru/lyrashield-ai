...
The scan engine is a separate Python repository at `ecryptoguru/lyrashield-engine`. It is a **controlled derivative** over pinned Strix v1.5.3 at `7cc9fa9faa0179fc7e35111102fe3d20a9028393`, not a repository-wide rebrand or a thin wrapper. Product behavior lives outside `strix/**`; only two hard-gated generic registration seams remain in the retained upstream tree.
...
5. Build the locked engine into the worker image from the sibling canonical repo, publish and verify its immutable digest, then promote that digest explicitly on the dedicated worker VM with a retained rollback digest; never follow a mutable worker tag.
...
Recorded release/base: v1.5.3 / 7cc9fa9faa0179fc7e35111102fe3d20a9028393
Engine version: 1.2.0
Offline proof: `scripts/verify-controlled-derivative.sh` passes the footprint, Ruff, formatting, pytest, mypy, Bandit, package/native-binary, sandbox, and worker-compatibility gates
...
Current worker policy: Safe/Quick use GPT-5.6 Luna at medium reasoning with a $1.20 cap; Standard uses Luna at medium with $3.20. Both have a 15-minute total ceiling (12 engine + 3 deterministic scanners). Deep/Custom use a GPT-5.6 Terra/medium root coordinator and Luna/high child specialists with a $5 cap and 45-minute total ceiling (40 engine + 5 scanners). A finite positive `Policy.maxBudgetUsd` or duration can lower but never raise the profile ceiling. `LYRASHIELD_LLM` is the fallback if a selected deployment variable is unset. Child work is deterministically assigned to Luna; the model cannot self-promote specialists to Terra, and only the root can create or stop specialists.
...
A threat model must be maintained as either `docs/security/threat-model.md` or a customer-declared, versioned target threat model in the private assurance workspace, and reviewed before each major release. Neither artifact is independently verified by LyraShield solely because it is complete.
...
- Prisma migration `20260803000001_child_table_rls` defined fail-closed RLS policies for `ScanEvent`, `Evidence`, `ScorecardShare`, `ScorecardEvent`, `ReferralCode`, `ReferralAttribution`, `NotificationPreference`, and `OnboardingState`, and `20260803000002_child_table_rls_enable` enabled them. `20260803000003_child_table_rls_disable` then disabled RLS on these nine child tables after a production `42501` outage, and **`20260807000003_child_table_rls_re_enable` restored it on 2026-08-07** once the cause was traced to `account-deletion.ts` writing to `ScorecardShare` outside `withWorkspaceRLS`. Database-level RLS coverage is now the full 30 tables. Application-layer enforcement remains as defence in depth, and the `rls-child-write-repro` CI job guards the child tables on every push; see Deep Review v12 P0-1.
...
- Current Docker proof: fresh web/worker images build; the web container passes health plus database/Redis readiness; scan readiness reports the live worker heartbeat; and the pinned engine-bearing worker image reports CLI `1.2.0`. The PR #115 smoke retained zero enabled schedules, queued database scans, waiting jobs, or active jobs before and after restart. Docker/runtime health remains separate from a paid controlled-scan result.
...
- Current merged-engine proof: engine PR #58 completed the version 1.2.0 product-outside-Strix migration over pinned Strix v1.5.3. `scripts/verify-controlled-derivative.sh` and current CI output are authoritative for the footprint, quality, sandbox, and worker-contract gates.
- Engine ownership boundary: LyraShield Engine keeps product behavior in `lyrashield/**` and `lyrashield_adapter/**`; the retained `strix/**` tree has only two hard-gated generic registration seams (+24/−0) against `7cc9fa9faa0179fc7e35111102fe3d20a9028393`. LyraShield owns GPT-5.6 policy, budgets, compaction, non-interactive lifecycle, deterministic identities, evidence/result schema, and the worker protocol; upstream remains the substrate for generic sandbox, tool, and agent-SDK plumbing. A fully independent rewrite is not justified without repeated upstream blockers or evaluation evidence.
...
- **URL/API scan capability profiles (PRs #272–#273, 2026-08-10):** URL and API targets use the versioned `url-scan/2.0.0` capability registry (`packages/types/src/url-scan-capabilities.ts`) with six released profiles — `WEB_APP_SAFE` (Surface Review), `WEB_APP_STANDARD` (Expanded Surface Review), `WEB_APP_DEEP` (Behavioral Surface Review), `API_SAFE` (Endpoint Review), `API_STANDARD` (Contract Review), and `API_DEEP` (Contract Behavior Review) — each carrying bounded limits for documents, assets, depth, bytes, concurrency, wall time, operations, method probes, and origin probes. `packages/security/src/public-surface.ts` is the shared SSRF-safe public-surface collector; `packages/security/src/public-surface-analysis.ts` unifies header/transport/cookie/secret detection; Standard web discovery adds same-origin BFS, `/robots.txt`, `/sitemap.xml`, and source-map references; Deep web behavior probes (`apps/worker/src/engine/scanners/url-behavior-probes.ts`) issue bounded HEAD, OPTIONS, and alternate-origin GET requests and detect reflected-origin CORS with credentials. API Contract and Contract Behavior reviews (`apps/worker/src/engine/scanners/openapi-scanner.ts`) parse OpenAPI 3.x specs and run safe operations bounded by the profile. `Target.apiSpecUrl` is validated as public HTTPS with no credentials and is required for API STANDARD/DEEP. `/api/scans`, scan-presets, and the schedule runner enforce target/mode parity. See `codebase.md` §67.
...
- Azure AI / GPT 5.6 routing is wired through `LYRASHIELD_LUNA_LLM`, `LYRASHIELD_TERRA_LLM`, and the fallback `LYRASHIELD_LLM`, sharing `AZURE_AI_API_KEY`, `AZURE_AI_API_BASE`, and `AZURE_API_VERSION`/`LLM_API_VERSION`. Safe/Quick/Standard use Luna at medium reasoning throughout; Deep/Custom use a Terra/medium coordinator with Luna/high specialists. The configured Azure Foundry endpoint accepts baseline Responses and `previous_response_id`, but rejects `programmatic_tool_calling`; direct JSON function tools are the production default and the programmatic flag remains opt-in behind the engine provider-contract gate. The current SQLite session architecture cannot combine with `previous_response_id`, so server-managed continuation remains unimplemented. Default dollar caps are $1.20/$1.20/$3.20/$5/$5, with positive workspace-policy values clamped to the selected profile ceiling and `PLATFORM_MAX_SCAN_BUDGET_USD` (default $50). The private ledger retains engine telemetry, actual-model per-request standard/long-context and cache-read/cache-write buckets, applies the versioned official GPT-5.6 rate card only when the bucket boundary is known, and refuses to invent a precise value from ambiguous aggregates. Accounting and cap events are private and no cost/spend amount appears in the dashboard. An overage is retained as usage truth, transitions the scan to `STOPPED_BUDGET`, and cannot persist billable cents above the approved cap.
- App PR #113 and engine PR #7 complete the coordinated AI-pipeline contract: only GPT-5.6 Terra/Luna deployments are accepted; the inherited Perplexity integration and other non-OpenAI model-provider credentials remain outside the worker boundary; context, output, agent count, and pre-request spend are bounded; raw engine output is not persisted; usage is checkpointed before deterministic scanners; structured evidence and corroborating receipts survive exact finding identity; and model-only coverage without explicit control IDs remains inconclusive.
- **Parallel Search `web_search` integration (2026-08-04):** the engine now exposes an optional `web_search` tool backed by Parallel Search when `LYRASHIELD_WEB_SEARCH_ENABLED=1` and `LYRASHIELD_WEB_SEARCH_API_KEY` is present. The engine redacts target hosts, secrets, IPs, and PII before sending a query, limits calls with `LYRASHIELD_WEB_SEARCH_MAX_CALLS_PER_SCAN`, and tracks cost against a separate `LYRASHIELD_WEB_SEARCH_BUDGET_USD`. The worker's `buildEngineEnv()` allowlist, `packages/config`, `.env.example`, `docker-compose.yml`, and production `ops/worker` scripts have been updated to pass `LYRASHIELD_WEB_SEARCH_*` variables; `ops/worker/refresh-egress.sh` adds `api.parallel.ai:443`. The tool is available to all scan modes while per-mode gating is evaluated.
- The versioned Vibe Security 50 registry delivers 43 code/URL review controls to the existing engine and preserves an explicit 7-control evidence-required boundary. It adds bounded deterministic SCA, secret, URL, agent-instruction, and CI-workflow signals without treating no result as a pass. See `docs/vibe-security-50.md`.
...
- PR #69 makes the public product narrative evidence-backed release assurance: `Target → Scan → Evidence State → Fix Proposal → Retest → Assurance Report`. The homepage, methodology page, tool funnel, and claim-regression tests distinguish detected candidates, independent verification, retest-confirmed validation, and inconclusive results. Tool signup attribution accepts only known `source=tools|tool` values and otherwise falls back to landing. The header omits the sign-in link when no app origin is configured for a preview build.
...
### C1.18 Blog program batches 7–10, the compare collection, and the outcome-forward homepage rewrite (2026-08-07 to 2026-08-08, 13 PRs merged, `main` at `476fcc0`)

- **Content program is 161 published blog articles, not 100.** The C1.16 authority-blog release (PR #88) mapped exactly 100 articles across seven releases (`authority` through `batch-6`); this program added batches 7–10 across five merged tranches (PRs #227, #230, #232, #233, #234, #235) covering 15 new coding-agent integration checklists, their 15 workflow companions, a refresh of the 15 pre-existing integration posts to open-registration/CLI-first positioning, and 13 comparison-vs-competitor blog posts. `apps/marketing/src/content/blog-program.json` remains the single membership source of truth; `PROGRAM_ARTICLE_COUNT` in `apps/marketing/scripts/blog-validation-lib.mjs` is now `161`, not a hardcoded `100`.
- **The image library is 75 entries, not 36.** `IMAGE_CORPUS` in the same file declares `{ authority: 1, shared: 74 }`. The original exact `29×3 + 6×2` reuse histogram (C1.16) no longer holds at this size; the enforced invariant going forward is that a shared image's declared `usageCount` must equal its actual assignment count (checked on every commit) and every image's usage falls inside `1..MAX_SHARED_IMAGE_USAGE`, not one fixed distribution shape.
- **A new sibling content collection, `/compare`, was added and is governed the same way** (PR #229 built it; PR #234 populated it). `src/content/compare/*.md` plus `src/content/compare-program.json` are the manifest and membership source of truth for 13 competitor-comparison pages, each backed by a comparison blog post with a bidirectional link. `compare:validate` is the release gate, parallel to `blog:validate`.
- **Two new offline CI gates were added** after real defects reached `main` under the prior single-validator setup: `blog:validate:mdx` (PR #228) catches MDX authoring hazards — an unterminated inline-code span that lets `${VAR}` or a JSON object leak into a live MDX expression, which compiles cleanly and only fails at prerender with no file name in the error — and `blog:validate:offline` (PR #236) catches a small, maintained denylist of known-dead external URLs (`DEAD_URLS` in `apps/marketing/scripts/blog-offline-gates.mjs`) plus unquoted YAML frontmatter scalars containing `": "`, which `astro check`'s real YAML parser rejects but the hand-rolled `parseArticle` accepted. Both run in CI on every commit; `blog:check-links`, which resolves ~50 live third-party URLs, deliberately does not, because a transient host timeout would make merges depend on third-party uptime.
- **Publishing is tranche-merge, not draft-flag scheduling.** `draft: true` cannot stage a program-mapped article for a later date: it fails the `draft === false` release check, breaks the program-completeness test's published-count assertion, and (because the cross-link validator only resolves `/blog/` links against non-draft posts) cascades into "unpublished internal dependency" failures for every published post that links to it. `pubDate` has no publish-gating effect anywhere in the codebase — it is sort order and RSS metadata only, so a future date publishes immediately and sorts to the top. The verified mechanism is that each merge to `main` is a complete, independently green release tranche; the merge calendar is the publish calendar.
- **The homepage was rewritten (PR #238)** to an outcome-forward hero — H1 `"Ship AI-built apps with evidence, not hope."`, dual CTAs (Lite Check anchored to the on-page `#free-scan` form, and account creation), explicit who-it's-for and MCP-differentiator lines, and honest open-beta/roadmap framing throughout. `lib/motion-manifest.ts`'s seven-chapter copy was reworded in the same pass (chapter `id`s, which key the R2 motion media paths from C1.16, are unchanged); a `max-width: 480px`-scoped CSS fix keeps the primary CTA above the fold at 390px without touching desktop layout.
- **Content guardrails enforced throughout:** no security guarantees, no invented metrics/customers, approval-gated fix proposals described as "not unattended fixes" (never "automatic fixes"), and the four-state evidence taxonomy (detected / independently verified / retest-confirmed / inconclusive) held exact. `PROHIBITED_CLAIMS` and `PLACEHOLDERS` in `blog-validation-lib.mjs` are shared between the blog and compare validators.
- **Not yet done:** the `blog:check-links` live sweep is run manually, not on a schedule; there is no CI workflow that would catch newly-dead external URLs between manual sweeps other than the fixed `DEAD_URLS` denylist.

### C1.19 OAuth/legal routes, MCP endpoints, and marketplace v0.1.8 (2026-08-09, PR #247, `main` at `dae5153`)

- **Hosted OAuth 2.0 for remote MCP clients is merged.** Remote MCP clients (`/api/mcp`) now authenticate via hosted OAuth with workspace selection and optional write scope, in addition to the existing `lsk_` API-key bearer flow. New routes: `/oauth/consent`, `/oauth/select-workspace`, `/device`, `/device/approve`, `.well-known/oauth-authorization-server`, `.well-known/oauth-protected-resource` (root and `/api/mcp/`). The OAuth provider is persisted via migration `20260808000000_oauth_provider` and the `OAuthClient`/`OAuthConsent`/`OAuthToken` models. Remote connections are read-only by default; write actions require explicit scope and approval. OAuth clients can never use the `LYRASHIELD_MCP_ALLOW_REMOTE_MUTATIONS` bypass.
- **CLI OAuth device login is merged.** `packages/cli` added a `login` command that opens a browser-based device approval flow, writes tokens to `~/.lyrashield/credentials.json` (0o600), and falls back to `LYRASHIELD_API_KEY` from the environment. `packages/credentials` is the single source of truth for that file, shared by the CLI and MCP server.
- **`@lyrashield/agent-plugin` package is merged** (Agent Plugins v1.0.0, Apache-2.0). Packages the MCP server and skills into a portable plugin with `plugin.json`, `mcp.json`, and `skills/lyrashield/SKILL.md`, plus client-specific manifest shims for Claude, Cursor, Codex, and Kiro. `packages/agent-plugin/src/export.ts` generates the deterministic marketplace release boundary.
- **Marketplace v0.1.8 is tagged and released** at `ecryptoguru/lyrashield-marketplace` (tag `v0.1.8`). The export reconciles the drifted v0.1.7 with current source, including the corrected Codebuff publisher slug (`lyrashield`), refreshed OAuth/legal link metadata, and the Apache-2.0 plugin license.
- **Legal/support pages are merged.** `/privacy`, `/support`, and `/security-reporting` are live on the marketing site.
- **Brevo email integration is verified locally.** `LYRASHIELD_REQUIRE_EMAIL_VERIFICATION=1` with `BREVO_API_KEY`, `EMAIL_FROM`, and `NOTIFICATION_FROM_EMAIL` boots cleanly in `NODE_ENV=production`; a live test email was sent and accepted by Brevo. Brevo IP security is disabled at the account level because Azure Container Apps Consumption has 180+ dynamic outbound NAT IPs. Production Container App secrets are not yet provisioned — the production app still runs with `LYRASHIELD_REQUIRE_EMAIL_VERIFICATION=0`.
- **Full release gate:** core, marketing, motion, and E2E suites plus lint, typecheck, format check, migration checks, and the `apps/web` production build must all be green. Current command output is authoritative.

### C1.20 Marketplace/CI/OAuth hardening and agent-plugin packaging (2026-08-10, PRs #249–#264, `main` at `d3b9868`)
...
- **Remove unused `apps/agent` package (PR #252, `b275ad7`).** The `apps/agent` application, package manifest, source, and tests are removed; `.github/scripts/classify-paths.sh`, `.github/workflows/ci.yml`, `README.md`, `Dockerfile`, `turbo.json`, and `packages/agent-plugin/src/export.ts` are updated to stop referencing it.
...
### C1.21 AI App Security scanner (Release A) and AI safety eval harness (2026-08-13)

- **Deterministic AI-layer static-analysis scanner.** A new "AI App Security 8" control set maps eight statically detectable signals to the OWASP Top 10 for LLM Applications (2025): AI-01 (prompt-injection input validation, LLM01), AI-02 (sensitive data in LLM context, LLM02), AI-03 (AI library supply chain, LLM03 partial), AI-04 (LLM output in dangerous sinks, LLM05), AI-05 (unbounded agent permissions, LLM06), AI-06 (system prompt exposed to client, LLM07), AI-07 (unauthenticated vector DB / RAG access, LLM08 partial), and AI-08 (missing LLM consumption limits, LLM10). The shared browser-safe core lives in `packages/security/src/ai-security/` and exposes `scanAiSecurityFiles`, `summarizeAiSecurityCoverage`, `computeAiSecurityScore`, and `AI_SECURITY_DETECTOR_VERSION`. Result states are `DETECTED`, `NO_FINDING`, `INCONCLUSIVE`, and `NOT_ASSESSED`; unsupported or truncated input cannot become `NO_FINDING`. The control set is separate from the Vibe Security 50.
- **Free browser-local product-led SEO tool.** `/tools/ai-app-security-scanner` runs AI-01, AI-02, and AI-04 through AI-08 entirely in the browser with no account, upload, advisory lookup, or LLM. Accepts pasted code or selected local files (≤25 files, 1 MiB/file, 5 MiB total). Renders evidence-state counts, control mappings, bounded evidence, remediation, supported scope, limitations, and a primary CTA to "Create account and scan the complete repository." AI-03, full-repository context, optional LLM triage, persistent evidence, retest, reporting, and the numeric score are gated to the paid repository scan. The free tool shows no numeric score because selected-file coverage is user-controlled and incomplete.
- **Paid full-repository deterministic scanning and AI-03.** The worker reruns the same core across a repository snapshot. AI-03 is only eligible for a clean outcome when exact supported dependency resolution and a complete fresh advisory receipt exist; stale, unavailable, partial, or unresolved data is inconclusive. The final SCA unification, advisory fixture corpus, and release verification remain in progress.
- **Optional bounded LLM triage overlay.** The generic `packages/security/src/ai-security/triage.ts` helper is reusable validation code, not the released production integration. Production triage remains blocked pending the controlled engine command, redaction receipt, accounting, budgets, provenance pin, and contract tests.
- **Private paid AI App Security Score.** `computeAiSecurityScore` is a pure private score with a minimum assessment gate, complete fresh AI-03 prerequisite, and no public sharing. Snapshot isolation and immutability are implemented, but disposition carry-forward, final reporting UX, and release proof remain in progress; do not claim a generally available scored product until those gates are complete.
- **Paid dashboard AI score card and API.** Authenticated `/api/scans/[id]/ai-score` route and `AiSecurityScoreCard` dashboard component render the score beside "assessed N of 8," evidence-quality counts, methodology version, advisory freshness, and limitations. `LIKELY_FALSE_POSITIVE` remains scored until an authorized user records false positive with a reason; accepted risk remains scored.
- **Private reports and conversion attribution.** `packages/db/src/report-generator.ts` adds a private AI App Security Score section with non-certification disclaimers. AI score, coverage, and provenance are added to private reports without exposing model cost or private accounting. Signup attribution uses only the existing privacy-safe source/channel mechanism.
...
- **Evidence storage.** `packages/evidence-storage/` provides append-only, private, encrypted artifact storage with workspace isolation using `withWorkspaceRLS` and `FORCE RLS`. AI-assurance data is never exposed in public report/share payloads.
...
- **Provisioning evidence (2026-08-21):** Polar/Razorpay test credentials, product/price IDs, webhook secrets, signed webhook smoke, and non-charge test objects are configured. This does not activate or prove live paid collection; a founder-controlled paid-launch decision and live provider-event evidence remain separate.
...
3. **Production infrastructure:** production PostgreSQL, dedicated worker compute, the authenticated application origin, secrets, backup/restore, and the worker-only egress proxy are live. The current BullMQ Redis endpoint remains public and non-TLS; an attempted source-IP restriction using the Container Apps environment's displayed static IP caused a new app revision to time out and was rolled back. A private/TLS-compatible replacement is required. Mandatory private S3-compatible evidence proof, monitoring/capacity evidence, and the remaining infrastructure gates also stay explicit. Apply every committed migration and replay the complete migration directory on a fresh database; command output and the directory are authoritative rather than copied counts. This includes scorecard events, approvals, evidence/result integrity, accounting, provider uniqueness, finding status reasons, UX state, OAuth/provider binding, API-spec support, and the final child-table RLS sequence. Reconcile legacy duplicate provider bindings before the uniqueness migration; evidence persistence fails closed until the configured `S3_*` endpoint succeeds.
...
- **Provision `BREVO_API_KEY`, `EMAIL_FROM`, and `NOTIFICATION_FROM_EMAIL` as production secrets on the `lyrashield-app` Container App and set `LYRASHIELD_REQUIRE_EMAIL_VERIFICATION=1`.** A Brevo API key is provisioned and verified locally (2026-08-08); the local production-mode boot and a live test email both succeed. Brevo IP security is disabled at the account level because Azure Container Apps Consumption has 180+ dynamic outbound NAT IPs. The production Container App does not yet have the Brevo secrets set, so production registration still accepts unverified addresses. See `docs/deployment/PRODUCTION_DEPLOYMENT.md` "Known production blockers" §1.
...
| Model routing and accounting | Complete in app code               | Safe/Quick/Standard route to Luna/medium; Deep/Custom use Terra/medium coordination with Luna/high specialists; protected limits reach the engine; actual-model per-request standard/long-context/cache-read/cache-write buckets drive private reconciliation; the dashboard exposes no costs. Azure Foundry uses direct JSON function tools unless its explicit programmatic-tool gate passes; server-managed continuation is not implemented while SQLite sessions own multi-turn state. Engine context compaction and GPT-5.6 execution hardening are merged; controlled production proof remains. |
