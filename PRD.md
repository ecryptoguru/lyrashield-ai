...
>
> ⚠️ **2026-07-06 UPDATE (authoritative over older status lists):** (1) **Repo renamed** to `github.com/ecryptoguru/lyrasec-ai` — in-code `@lyrashield/*` package scopes and `LYRASHIELD_*` env vars are intentionally NOT renamed yet (trademark clearance still open; revert risk). (2) **v1 coverage FINAL:** agentic pentest **+ SCA + secrets** + a first-class GitHub Action / reusable workflow with a diff-aware gate + SARIF output (resolves founder decision #15). IaC/container + reachability remain Phase 2. (3) **Batches 1–4 complete + Sprints 4–7 complete + UI/UX refinement sweep complete + AI pipeline audit complete + Docker deployment verified + Agent Action Layer complete** (781 tests, 62 files, all green). Sprint 5: engine binary wired. Sprint 6: findings normalization (severity, CWE, CVSS, confidence, dedup). Sprint 6.5: SCA scanner (7 formats, OSV API) + secrets scanner (12 patterns) + scanner orchestrator. Sprint 7: AI-builder-aware URL scanner (10 detectors) + launch-readiness UI + shareable report/badge + MCP server (real API calls, stdio transport) + prompt-injection defense (27 patterns) + GitHub Action diff-gate. UI/UX: raw `<label>` → `FormField`, raw colors → design tokens, `aria-hidden` on icons, `tracking-tight` on headings, `Spinner` in loading states. Docker: all 5 containers build/run, 18 pages return correct HTTP codes (200/307/404), 13 API endpoints respond correctly (400/401 for unauthenticated/missing params), 7 migrations applied, 18 RLS tables confirmed, worker ready with BullMQ, Next.js 16.2.10, 781 tests pass inside container. Dockerfile updated to include `apps/agent` package. Agent Action Layer (Sprint 3.5 + 7.6): `AgentApproval` model + RLS (18th table), `apps/agent` package (service token, ActionRegistry, 6 actions, plain-language bridge, scan queue enqueuing), agent permissions, approval API routes. Deep code review applied: 7 fixes (approval verification, audit fault isolation, scan enqueuing, token payload validation, static imports, policy validation, deny docs). **See `codebase.md` §22/§23/§24/§25/§26/§27 for full details.** **Remaining:** billing (Sprint 10+), enterprise features (Sprint 12-19), Security Copilot sidebar (Sprint 5.5), Visual Security Plan (Sprint 8.5). See §B13.6 for details.
...
>
> ⚠️ **2026-07-11 THIN-FORK RELEASE-GATE UPDATE:** The engine now has a weekly/manual, review-only upstream-sync workflow: it verifies ancestry, aborts rewritten history with exit `20`, rebases and verifies only when needed, and opens an `automation/upstream-<short-sha>` PR—never auto-merging, force-pushing, or resolving conflicts. Baseline is `7b639505fecf20a2d9e356f96bd91470aa828182`; the adapter maps product variables only when an explicit upstream `STRIX_*` value is absent and telemetry defaults off. Worker compatibility covers both `strix_runs` and `lyrashield_runs`. Current offline proof is 155 engine tests plus Ruff/format/mypy/Bandit and 601 application tests plus lint/typecheck/build. The worker Docker build now completes after scoping Next.js compilation to `@lyrashield/web`; local image ID `sha256:71d6c104f5d11e30d8f8ee63cef8aacb1819b5ec8a4c3d1987d7fd3dcaddc4e6` passed `lyrashield --version` (`1.0.4.post1`) and missing-model validation before any sandbox pull. The inspected production sandbox image is `ghcr.io/usestrix/strix-sandbox@sha256:478e0b37ec83b2ba8c6e159593cb46d5dc9b624a45d6a9bb606851b83058d284`. Engine `origin` is now `ecryptoguru/lyrashield-engine`, with the reviewed thin fork in [PR #1](https://github.com/ecryptoguru/lyrashield-engine/pull/1). No sandbox execution or controlled scan is claimed; authorized LLM configuration is required before one can run. See `codebase.md` §30.
