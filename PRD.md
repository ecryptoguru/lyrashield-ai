...
- Monorepo: 5 apps (`web`, `worker`, `agent`, `marketing`, `marketing-motion`) and 16 shared packages (`agent-plugin`, `agent-registry`, `agent-rules`, `auth`, `cli`, `cli-alias`, `config`, `credentials`, `db`, `integrations`, `logger`, `mcp`, `score`, `sdk`, `security`, `types`, `ui`).
