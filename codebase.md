...
>
> **Current baseline — 2026-07-16:** 4 apps, 10 shared packages (including `packages/score`), 22 web page files, 40 API route files, 39 Prisma models, 18 enums, 18 migrations, and 20 directly RLS-protected workspace tables. Current `main` passes lint, typecheck, test, E2E, production build, formatting, Prisma client generation, and diff checks (818 tests in 85 files; 2 Playwright Chromium tests). Sections 17–42 are dated implementation history; their older counts are checkpoints, not the current gate.
...
| Testing                 | Vitest + Playwright              | 778 tests in 77 files + 2 Chromium E2E tests         |
...
- `pnpm test`: **778 tests in 77 files**, pass
...
- Prisma validation, deployment, and status: pass; the repository contains all 18 committed migrations
- `git diff --check`: pass
...
- Marketing is deployed and indexable at `https://lyrashieldai.com` with production D1/Rate Limit/KV bindings, all D1 migrations, a Worker-secret IP salt, custom apex/`www` domains, an active canonical 301, sitemap/robots/`llms.txt`, security headers, and live waitlist/crawl/Lighthouse/Brave QA. `/scan` and `/terms` remain individually `noindex`; the separate public app origin, Turnstile, abuse mailbox, and app-origin unfurl/referral proof remain open gates.
...
- Branch and PR validation passed 822 tests in 86 files, two Chromium E2E tests, lint, typecheck, production builds, formatting, Prisma generation, Docker artifact smoke, Worker-backed D1/API checks, and `git diff --check`. Local Worker Lighthouse scored 99/100/100/100 mobile and 100/100/100/100 desktop. Main-branch deployment and post-deploy live checks remain a separate final proof gate until this PR merges.
