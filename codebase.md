...
> Current implementation map: 2026-09-25 (post Deep Review v20 and the connectors, Myra and GPT-6 routing waves). Read [AGENTS.md](./AGENTS.md) first for the immediate handoff and rules; use [PRD.md](./PRD.md) for product scope and release gates. Running code, Prisma schema, migrations, CI, and live evidence override this guide.
...
| `gate`             | Pure versioned Launch Gate standard (`lyrashield-gate/2.3.0`) and verdict computation      |
...
Protected routes use `{ success: true, data }` or `{ success: false, error: { code, message, requestId } }` and carry an `x-request-id` response header. Lists use cursor pagination. Client code reuses `apiGet`, `apiPost`, `apiPatch`, `apiDelete`, and `apiGetPaginated` rather than ad hoc fetch wrappers.
...
- The Launch Gate: `packages/gate` computes the verdict (`lyrashield-gate/2.3.0`, pure, versioned); `packages/db/src/gate-service.ts` persists append-only `GateVerdict` rows under RLS; uncovered target types always yield INSUFFICIENT_EVIDENCE. `packages/db/src/launch-report-payload.ts` is the ONLY public launch-report payload constructor (key-set regression test, same discipline as `buildScorecardPayload`).
- Launch report issuance (`launch-report-service.ts`) selects one verdict and evaluates its applicability at issue time inside a RepeatableRead snapshot (policy fingerprint, newer-attempt, evidence drift, freshness), never trusting the stored staleness flag; `stale` maps every non-applicable/unknown outcome. The report row's private `provenanceJson` binds the exact `gateVerdictId`, verdict checksum, assessed release identity (commit/artifact digest), and issue-time applicability — written atomically with the signed payload, readable on authenticated surfaces (`GET /api/reports/[id]` `launchReport` block, reports list). A recipient holding the signed report and its live share capability may submit an expected identity to `POST /api/reports/verify`; the confirmation-only response is `MATCH`, `MISMATCH`, or `UNAVAILABLE` and never returns the stored value. Legacy reports without provenance display "Release identity unavailable for this report".
- Launch Readiness supports a target-scoped release check: `GET /api/launch-readiness?workspaceId=&targetId=&commit=|artifactDigest=` (full 40-hex SHA or `sha256:`+64-hex digest, mutually exclusive, repeated params rejected) runs the gate evaluation against the requested identity and returns `releaseCheck` with match/mismatch/cannot-confirm plus requested vs assessed identities. The dashboard form drives the same URL (`/dashboard/launch-readiness?targetId=&commit=`); without a reference the view stays informational.
- `GET /api/billing/local-availability` is the public, origin-checked-CORS, rate-limited, no-store read the marketing pricing page uses to disclose Local purchase availability for the visitor's resolved provider before any checkout link. It is display state only — `/buy/local` and `/api/billing/local-checkout` re-check admission authoritatively.
...
Published npm versions checked on 2026-09-26: `lyrashield` CLI `0.2.11`, `@lyrashield/mcp` `0.2.9`, and `@lyrashield/agent-plugin` `0.1.27`; check the installed package's engine and runtime requirements. The registry contains 30 entries and resolves 26 preferred client surfaces: 13 config-file installs, one vendor CLI, seven guided-manual clients, and five Agent Plugin package paths for Claude Code, Cursor, OpenAI Codex, GitHub Copilot, and Kiro. Cursor accepts a direct local plugin copy; Codex, Claude Code, and GitHub Copilot require their marketplace flows; Kiro requires merging the generated stdio entry into its documented MCP settings file. The Codex installer registers the LyraShield marketplace before adding the plugin. OpenCode uses its documented global `~/.config/opencode/opencode.json` or project `opencode.json` location. The registry retains three legacy config-file alternatives for plugin-preferred clients. VS Code stays on its config-file path.

The unreleased agent-surface candidate advances the source packages to CLI `0.2.13`, MCP `0.2.10`, and Agent Plugin `0.1.30`. SDK v2 handles modern and legacy MCP exchanges; `AgentOperation` and scan IDs provide recovery, while MCP Tasks remain unadvertised because the official modern SDK receiver rejects them. SDK/CLI/MCP share advisory eligibility and bounded diff analysis. Scan waiting and explicit cancellation, attachment access, fix-PR recovery, and dashboard WebMCP progress/report tools are implemented in source. `docs/agent-surfaces-completion.md` records tests and the separate publication, deployment, and live-proof gates.

Hosted OAuth is read-only by default. A user can grant a connection a bounded set of workflows, targets, and scan profiles once during OAuth consent. Every delegated mutation then revalidates the connection, user membership, permission, scope, expiry, target, profile, and caller-supplied idempotency key. Write-scoped API-key calls on the remote endpoint receive a `connect_required` response and delegated calls outside the grant are denied; exact-input approval remains only for legacy nondelegated hosted credentials.
