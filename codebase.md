...
- **Redis remains a release gate.** A source-IP restriction using the Container Apps environment's displayed static IP made a new app revision time out when reaching Redis, so the rule was rolled back to preserve availability. The endpoint remains public and non-TLS; this is not evidence of the required private/TLS BullMQ architecture and must not be used to close that release gate.
