...
>
> **Current merged baseline — 2026-07-18:** 4 apps, 10 shared packages (including `packages/score`), 25 web page files, 42 API route files, 39 Prisma models, 18 enums, 21 migrations, and 20 directly RLS-protected workspace tables. PR #109 passes lint, typecheck, E2E, production build, formatting, Prisma client generation, migration drift/application, SCA/secret scanning, the security diff gate, CodeRabbit, and diff checks (858 core tests in 94 files, 79 marketing tests in 12 files, 16 motion tests, and 2 Playwright Chromium tests). Sections 17–51 are dated implementation history; their older counts are checkpoints, not the current gate.
...
- Engine PR #6 is a separate trust boundary: it adds estimated-input compaction at 240k toward 180k and bounds direct dedupe input to 200 kB. Its CI is green, but this document does not call it engine-`main` behavior until the independent-review branch rule is satisfied and the PR merges.
