...
- Prisma migration `packages/db/prisma/migrations/20260803000001_child_table_rls/migration.sql` defined fail-closed RLS policies for `ScanEvent`, `Evidence`, `ScorecardShare`, `ScorecardEvent`, `ReferralCode`, `ReferralAttribution`, `NotificationPreference`, and `OnboardingState`. `20260803000002_child_table_rls_enable` enabled them. **`20260803000003_child_table_rls_disable` subsequently disabled RLS on these tables after a production `42501` outage** — the policies exist in the migration chain but are inert. The fail-closed tests for these tables are skipped (`packages/db/src/rls-fail-closed.test.ts:185`). Tenant isolation is currently application-layer only; see Deep Review v12 P0-1.
