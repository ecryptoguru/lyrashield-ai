...
Current package/runtime contract: `lyrashield` CLI `0.2.3`, `@lyrashield/mcp` `0.2.5`, and `@lyrashield/agent-plugin` `0.1.23` require Node 24+. MCP uses SDK `1.30` and Zod 4. The registry contains 30 entries and resolves 26 preferred client surfaces: 13 config-file installs, one vendor CLI, seven guided-manual clients, and five Agent Plugin package paths for Claude Code, Cursor, OpenAI Codex, GitHub Copilot, and Kiro. Cursor and Codex accept direct local plugin copies; Claude Code and GitHub Copilot require their marketplace flows; Kiro requires merging the generated stdio entry into its documented MCP settings file. The registry retains three legacy config-file alternatives for plugin-preferred clients. VS Code stays on its config-file path.
