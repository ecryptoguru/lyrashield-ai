...
>
> **Current merged baseline — 2026-07-18:** 4 apps, 10 shared packages (including `packages/score`), 25 web page files, 42 API route files, 39 Prisma models, 18 enums, 21 migrations, and 20 directly RLS-protected workspace tables. PR #113 passes lint, typecheck, E2E, production build, formatting, Prisma client generation, migration drift/application, SCA/secret scanning, the security diff gate, CodeRabbit, and diff checks (869 core tests in 94 files, 79 marketing tests in 12 files, 16 motion tests, and 2 Playwright Chromium tests). Sections 17–52 are dated implementation history; their older counts are checkpoints, not the current gate.
...
- Prisma validation, drift, deployment, and status: pass; the repository contains all 20 committed migrations
- `git diff --check`: pass
