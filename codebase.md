...
The current production Standard/Luna acceptance scan exercised the deployed worker and engine with retained lifecycle events, findings, coverage and verification receipts, and a sealed result manifest. That target- and version-scoped run is not a security guarantee or proof of universal control coverage; Deep/Terra execution, private evidence-storage proof, monitoring, and capacity evidence remain separate gates.
...
| Engine adapter (5)                  | Code complete, release-gated | The controlled derivative and Docker integration pass deterministic gates; the current production Standard/Luna acceptance scan exercised the promoted image, model routing, accounting, receipts, and sealed result path. Deep/Terra proof, private evidence-storage proof, monitoring, and capacity evidence remain separate gates.                                                                                                                                                                           |
...
- **Managed TLS Redis and negative egress proof completed.** PR #380 moved BullMQ to the authenticated Upstash `rediss://` TCP endpoint while keeping the REST URL/token exclusive to rate limiting; PR #381 persists the scanner binding and managed-provider heartbeat; PR #382 makes the five-minute DNS-pin refresh defer a changed-pin restart while a scan is active. Worker digest `sha256:03f7a6b2d14bda224b14a9641a9acaeafa54c400f1b13fae3f002cdbb59e4f44` runs product revision `8f5257a96f5d8cbd299f5152215ac401ed16755c` with engine `dd588c379ae6614e0914b8adb41d94f0c1e86c26`. The current proof recorded `PONG`, zero queued/active jobs before acceptance, a live heartbeat, no Internet-facing Azure `6379` rule, and the stopped/restart-disabled legacy Redis rollback container. The worker denied a direct public fetch, allowed the authenticated proxy path, denied loopback with `ssrf_blocked`, and crossed timer refreshes during a billable Standard scan without restarting. Literal private-network Redis remains an enterprise topology option.
- **Current Standard/Luna production acceptance completed.** Scan `cmt35aj1s000001hck9fmguzk` ran `ecryptoguru/OnboardingAI2@1689f3607d68764e09769535df8e368c4d5ad2fe` through the promoted worker in 11m 42s. All 184 model requests used `azure_ai/gpt-5.6-luna` at medium reasoning; the private ledger recorded 8,227,004 input tokens (6,066,725 cached), 30,844 output tokens, and matching provider/billed cost of $0.597148 under the $3.20 cap. The run retained 24 findings, 24 candidates, 24 verification receipts, and 56 coverage receipts, debited 12 Standard agent-minutes, and stored manifest checksum `5813c6dc06bcb89b2386cb80563a93e928be96bfe7371a85c930704127606dec`. Zero findings were independently verified, and the AI App Security layer reached its 200-file bound, so this is bounded runtime/accounting proof rather than full coverage or a security guarantee.
