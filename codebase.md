...
- **Redis exposure reduction, not completion.** The worker VM NSG allows Redis only from the Container Apps environment's static egress IP and the worker VM. The endpoint is nevertheless public and non-TLS, so this is not evidence of the required private/TLS BullMQ architecture and must not be used to close that release gate.
