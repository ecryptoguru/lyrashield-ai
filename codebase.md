...
- **Bottom sheet animations:** `apps/web/src/components/bottom-sheet.tsx` uses enter/exit transitions with mount delay, opacity and translate-y CSS transitions, and delayed unmount for smooth animation.
- **Shared label module:** `apps/web/src/lib/enum-labels.ts` provides centralized human-readable labels for all Prisma enums (scan goals, modes, triggers, finding severities, statuses, verification statuses, target types). The duplicated `GOAL_LABELS` map in `scan-detail-client.tsx` was removed in favor of the shared module.
