...
describe("worker Docker runtime", () => {
  ...
  it("promotes the exact worker digest with bounded rollback and verification", () => {
    ...
    expect(workerPromoter).toContain("docker image prune --all --force")
    expect(workerPromoter).toContain('current_image_id=$(docker inspect "$container"')
    expect(workerPromoter).toContain("required_free=$((current_image_size * 3 + 2147483648))")
    expect(workerPromoter).not.toContain("docker system prune")
