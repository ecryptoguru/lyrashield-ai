...
async function readBoundedFile(repoPath: string, file: string): Promise<string | null> {
  try {
...
async function scanInstructionFiles(repoPath: string): Promise<EngineVulnerability[]> {
  ...
  for (const file of INSTRUCTION_FILES) {
    ...
    for (const [index, line] of content.split("\n").entries()) {
      ...
      if (/\b(?:do not|never|must not)\b/i.test(line)) continue
      if (!DANGEROUS_INSTRUCTION_PATTERNS.some((pattern) => pattern.test(line))) continue
      findings.push(
        finding(
...
async function scanWorkflows(repoPath: string): Promise<EngineVulnerability[]> {
  ...
  for (const fileName of files) {
    ...
    const hasWritePermission = writeAllLine >= 0
...
export async function scanAgentConfig(
  ...
  const [instructionFindings, workflowFindings] = await Promise.all([
    scanInstructionFiles(config.repoPath),
