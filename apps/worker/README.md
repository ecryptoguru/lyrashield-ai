...
Repository profiles are resolved from `@lyrashield/types`: Safe and Quick are the same Luna profile ($1.20, typically 15 minutes), Standard is Luna ($3.20, typically 15 minutes), and Deep/Custom is Terra/medium with Luna/high specialists ($5, typically 45 minutes). These are estimates, not elapsed-time engine cutoffs: the worker continues while the bounded engine receipt advances, and stops only on cancellation, protected budget exhaustion, or 20 minutes without durable receipt progress. Web-app/API profiles remain deterministic and do not invoke an AI model.
