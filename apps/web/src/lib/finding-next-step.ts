...
export function getFindingNextAction(input: {
  ...
  if (input.operationInFlight) {
    return {
      ...
      reason: "An operation for this finding is already running. Its status is the next step.",
    }
  }
  ...
  if (status === "FALSE_POSITIVE" || status === "DUPLICATE") {
    return {
      action: "NONE",
      reason: "This finding is dispositioned; no further action is required.",
    }
  }
  ...
  if (
    input.hasFixProposal === false &&
    input.latestRetestStatus !== "passed" &&
    input.hasEvidence
  ) {
    return {
      action: "INSPECT_EVIDENCE",
  ...
  switch (step) {
    ...
    case "FIX_PROPOSAL":
      return {
        action: "FIX_PROPOSAL",
        reason: "Generate a fix proposal from the retained evidence.",
      }
    case "RETEST":
