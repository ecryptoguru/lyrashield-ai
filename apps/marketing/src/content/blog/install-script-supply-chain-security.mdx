...
{
  "allowScripts": {
    "reviewed-native-addon": true,
    "unexpected-helper": false
  }
}
```
...
Use these fields only with an npm version that supports them and keep the version pinned in CI. Review what each allowed package runs and why. A package name on an allowlist is not permanently trustworthy. Recheck scripts when the resolved version or maintainer changes.
...
Bind the approval record to the resolved package version and lockfile hash. If the project supports several operating systems, inspect the artifacts and optional dependencies for each production platform. A Linux CI run may never reveal the script or binary selected on macOS or Windows.
