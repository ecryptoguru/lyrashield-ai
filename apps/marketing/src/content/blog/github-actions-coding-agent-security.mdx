...
The browser-local [secret exposure scanner](/tools/secret-exposure-scanner) can inspect selected workflow and configuration text for common credential patterns. It does not read repository settings, resolve action pins, inspect token permissions, or test a cloud trust policy.
...
The LyraShield AI repository contains a diff-gate workflow that resolves exact base and head commits, runs secret and dependency checks, produces SARIF, and makes a separate severity decision. It is implemented repository code, not a claim of a generally available hosted service. Its current `DEEP` input has SAFE-equivalent coverage and must not be described as a deeper scan. The [evidence methodology](/methodology) explains why coverage and limitations stay attached to a result.
