...
faq:
  - q: "Is a Replit Preview URL private?"
    a: "Do not assume it is private. Development URL visibility depends on workspace and policy settings. Protect sensitive applications with authentication and verify the URL from a signed-out browser."
...
Replit makes it easy to move from editor to a public URL, but Preview and publication do not run under one identical boundary. Replit documents separate development and deployment domains, snapshot-based publishing, deployment-specific secrets, and several compute types. A route that works in Preview can disappear, use the wrong callback, or reach a different data store after publication.
...
| Visibility    | Workspace policy             | Public or private deployment setting       |
...
Private deployment availability and development URL policy vary by workspace and plan. Describe the behavior you verified, not a universal entitlement. Application authentication and resource authorization still matter inside a private deployment.
