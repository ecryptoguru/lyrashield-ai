...
Model Context Protocol makes tools discoverable to a model. That is useful, but discoverability can hide the real permission question. A tool named `workspace_admin` with a flexible schema may expose filesystem, database, and network authority behind one apparently convenient call.

Start with a permission inventory: caller, tenant, resource, verb, fields, side effects, credential, approval rule, and audit result. This inventory supports the [approval-gated agent security model](/blog/vibe-coding-security-guide) by showing which authority exists before any prompt chooses a tool.

## The vulnerable pattern: one broad administrative tool
...
Generated MCP servers often start this way because one generic handler is easy to demonstrate. The model chooses an action string and the server forwards it to an SDK. The implementation may validate JSON shape while failing to constrain tenant, resource, field, or business state.
