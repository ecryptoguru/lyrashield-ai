---
title: "Vibe Coding Threat Model: Template and Review Checklist"
description: "Build a threat model for your AI-built app with a worked example, reusable risk table, named owners, and release checks tied to evidence."
pubDate: 2026-07-17
...
## Copy this threat-model review template

Use one row per concrete failure path. Replace the example values with facts from your app; blank evidence means the control has not been demonstrated.

| Field | Example for a workspace export |
| --- | --- |
| Flow and boundary | Browser to export API to worker to private storage |
| Asset and actor | Workspace documents; signed-in non-member |
| Threat | Changing a workspace ID exposes another tenant's export |
| Control | Server checks current membership before queueing and download |
| Negative test | Synthetic Account B requests Account A's export |
| Evidence | Test result, commit, environment, and redacted audit reference |
| Owner and decision | Named reviewer; block release until denial is demonstrated |

Review the download as well as job creation. A protected export API can still produce a publicly readable file. The [secure report-sharing guide](/blog/share-security-report-safely) covers the resulting access boundary.
...
The UK's National Cyber Security Centre published [the vibe coding spectrum approach](https://www.ncsc.gov.uk/blogs/the-vibe-coding-spectrum-approach-to-ai-assisted-software-development) in June 2026. It connects greater autonomy and more sensitive work with stronger control and review. That is guidance for adjusting oversight, not a measurement that assigns one universal risk level to every AI-assisted project.
