...
You configure LyraShield once in `.mcp.json` (shared) or `.pi/mcp.json` (Pi-specific) using `npx -y @lyrashield/mcp` locally or `https://app.lyrashieldai.com/api/mcp` remotely with `lsk_` Bearer key per [picode integration guide](/docs/integrations/picode). The combination is about verifier isolation: Pi owns generation and orchestration, LyraShield owns verification evidence that survives mode transitions. The platform is live in open beta, with the scan engine and automated fix execution on the roadmap.
