...
Start with the [vibe coding threat-model template](/blog/human-review-threat-model-vibe-coding) to record owners, controls, and missing evidence. A leaked service credential can bypass database policies and require incident response. Review those boundaries together.
...
Deployment defaults deserve tests of their own. Check that storage is private unless intentionally public, debug routes and source maps are absent, production error responses do not expose secrets, TLS is enforced, and backups exist outside the primary failure domain. Configuration drift can invalidate a clean repository review. The [Vercel deployment guide](/blog/vercel-deployment-security) covers preview protection and private environment variables.
...
The report combines target, coverage, evidence provenance, unresolved findings, retest receipts, operational evidence, and known limitations. It supports a release decision without pretending to eliminate uncertainty. The [evidence methodology](/methodology) defines the states used in that record.

## How the Vibe Security 50 fits

The versioned Vibe Security 50 registry contains exactly 43 code/URL review controls and 7 evidence-required controls. The 43 can be requested through deterministic, hybrid, or engine-led checks where applicable; that does not mean every control is established on every run. A control counts as a finding only when a scanner or engine returns evidence. If nothing is reported, the control is not silently marked passed.
...
- The [AI App Security Scanner](/tools/ai-app-security-scanner) reviews selected local files and pasted code in the browser.
