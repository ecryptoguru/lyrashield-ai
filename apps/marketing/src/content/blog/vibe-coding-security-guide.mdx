...
Keep secrets on trusted servers. A value embedded in a browser bundle, mobile package, public environment variable, source map, or client request must be treated as public even if the interface calls it a secret. Passwords need a password-hashing function with an appropriate work factor, not reversible encryption or a fast general-purpose hash.

Session validation belongs on the server. Verify the signature or server-side session identifier, issuer, audience, expiry, revocation rules, and the binding between the session and the requested tenant. Cookies that carry session authority generally need `Secure`, `HttpOnly`, and an intentional `SameSite` policy. Those attributes help at the browser boundary, but they do not correct a server that accepts an invalid or mis-scoped session.
...
Password reset and email verification flows deserve their own tests. Tokens should be unpredictable, single use, short lived, stored safely, and bound to the intended purpose. Responses should not reveal whether an address has an account. Changing a password should have a deliberate effect on existing sessions, and the user should receive an actionable notification.

Two free local analyzers can help with narrow parts of this layer. The [Secret Exposure Scanner](/tools/secret-exposure-scanner) checks selected text files for common credential patterns and redacts matches. It does not scan Git history, deployed assets, logs, cloud stores, or custom formats. The [JWT and Session Inspector](/tools/jwt-session-inspector) decodes a non-production JWT and reviews basic claims and cookie attributes. It does not verify the token's signature, issuer, audience, key, or revocation state. Never paste a production token into a diagnostic page.
...
Server-side URL fetching needs more than a URL parser. Restrict schemes and ports, resolve hostnames, reject private and special-purpose addresses, pin the permitted destination for the connection, and repeat the checks after every redirect. File uploads need size and type limits, generated storage names, isolation from executable paths, and safe downstream processing. Path operations should resolve against an approved root and reject escape attempts.
...
Review CI permissions as production permissions. A pull request from an untrusted fork should not receive a write token or deployment secret. Pin reusable workflow and action references where your threat model requires it. Separate build, test, and deploy identities, and make production promotion an explicit action with a reviewed artifact.
...
After a fix, run a fresh check against the changed system. Do not turn a manual status change into proof. A complete deterministic retest may show that the original condition is absent and support a retest-confirmed outcome. If coverage is incomplete or the original check depended on model judgment, absence may remain inconclusive. Preserve that uncertainty.

Operations supply evidence that source review cannot. Prove that monitoring reaches an owner, alerts are actionable, backups restore, queues recover, cancellations stop work, credentials rotate, and incident decisions have named responsibility. Test recovery before relying on it. A screenshot of a configured backup job is weaker than a retained restore record with timing and integrity checks.
...
Run the relevant check against the new state. Retest the abuse case, not merely the happy path. Also look for regression in neighboring controls. An authorization fix that blocks every user is not a successful security outcome.
...
The [free security tools hub](/tools) contains five browser-local analyzers. Inputs stay on the device, but that privacy boundary does not make the results comprehensive.
