---
title: "Firebase Security Rules: Firestore, Storage, and Admin SDK"
description: "Test Firebase Firestore and Storage rules with two accounts. Review Admin SDK bypasses, upload limits, and App Check before deployment."
pubDate: 2026-07-17
...
[Firestore server client libraries bypass Security Rules](https://firebase.google.com/docs/firestore/security/rules-conditions) and use IAM. A passing client Rules test therefore says nothing about an Admin SDK route. Authenticate the caller and authorize the specific document before making an administrative query.

Keep two test paths: call Firestore as Account B through the client SDK, then call your server endpoint as B with Account A's document ID. Both must deny access. For Cloud Storage, test the actual object path and operation under its own rules; a Firestore denial does not establish that an uploaded file is private.
