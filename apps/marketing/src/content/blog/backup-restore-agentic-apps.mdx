...
function mayExecute(approval: DestructiveApproval, evidence: RestoreEvidence) {
  return (
    approval.action === "apply-reviewed-migration" &&
    approval.artifactHash === evidence.reviewedArtifactHash &&
    evidence.status === "passed" &&
    evidence.environmentClass === "isolated-restore" &&
    new Date(approval.expiresAt) > new Date()
  )
}
```

This example does not grant a generic database administrator role. It binds an exact reviewed artifact, environment, expiry, and evidence record. The real implementation must validate all fields on a trusted server and consume the approval once. An agent must not be able to invent or alter the restore evidence it cites.
