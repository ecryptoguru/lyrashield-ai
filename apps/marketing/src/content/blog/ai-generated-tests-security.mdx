...
When one assistant writes both a function and its tests, the pair can agree on the same wrong behavior. The test passes because it restates what the implementation does, not because it checks what the system must protect.

This distinction matters in [how tests fit into release assurance](/blog/vibe-coding-security-guide). A green run is execution evidence. Coverage describes what ran. Independent verification asks whether a separate method supports the security claim. Those are different records.

## The vulnerable pattern: test only the generated happy path
...
An assistant can generate this pair from a request such as "add a test for getProject." It sees the function signature and expected return shape. Unless the prompt contains the ownership invariant, the shortest passing test follows the implementation.
...
For sensitive boundaries, have the reviewer add at least one case that was not proposed in the generation context. The case may still miss a flaw, but it reduces direct copying of the implementation's assumptions and makes the acceptance decision easier to explain.
...
When a generated suite passes, record the commit, environment, command, test selection, result, and limitations. If only unit tests ran with mocked storage, say so. Do not label the result "verified" without an independent verification receipt.
