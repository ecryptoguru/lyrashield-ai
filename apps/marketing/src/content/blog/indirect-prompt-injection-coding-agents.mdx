...
The risky text may sit in an issue body, README, source comment, generated documentation, web page, package metadata, or tool result. A user asks the agent to summarize or fix something. The agent reads the content, folds it into one context, and may follow an instruction that came from the content rather than the user.

The path is easier to reason about as a trust boundary:
...
## The vulnerable pattern: content and control share one channel
...
The label "Issue content" is helpful to a reader but does not create an enforcement boundary. The model can still use every exposed tool. Generated agent code often misses this because concatenating context is the shortest route to a working demo, while data provenance and action authorization live outside the model call.
