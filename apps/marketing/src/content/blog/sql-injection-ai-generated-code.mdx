...
const column = sortColumns[request.sort]
const direction = sortDirections[request.direction]
if (!column || !direction) throw new InvalidInputError()

const query = `select id, email from users order by ${column} ${direction}`
```

The SQL text still uses interpolation, but both inserted fragments come from fixed application constants. The request selects a mapping key and never becomes SQL text.
