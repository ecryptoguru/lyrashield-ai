...
faq:
  ...
  - q: Where does Codebuff keep its MCP config
    a: Codebuff uses .agents/mcp.json with a root mcpServers key holding command, args, and env for each server. Codebuff documents local stdio and remote HTTP servers.
...
Codebuff stores MCP server configuration in .agents/mcp.json with a root mcpServers key. Each server entry holds command, args, and env, and local stdio and remote HTTP transports are documented in the official guide. The [Model Context Protocol specification](https://modelcontextprotocol.io/) defines how these servers expose tools to an agent. Codebuff also supports an agent store where published agents can be added to a project, and the Codebuff CLI offers /init and /publish commands.
...
Codebuff may add or upgrade packages while implementing a feature. Do not trust the import list at face value. Inspect the final manifest and lockfile, confirm package identity and publisher, and review advisory results before installing in an environment with credentials. The [OSV.dev database](https://osv.dev/) aggregates known vulnerability data across ecosystems and the [npm security best practices](https://nodejs.org/learn) cover lockfile integrity for Node projects.
