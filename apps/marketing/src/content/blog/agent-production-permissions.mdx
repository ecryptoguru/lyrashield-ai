...
Production delete authority can hide behind database administration, cloud deployment, storage management, or a general CI role. An agent may receive that role because it needs one harmless operation. The same credential can then remove a table, bucket, deployment, environment, or audit record.
...
## The vulnerable pattern: inherited administrator access
...
AI-generated deployment workflows miss this boundary when they reuse the credential that made a manual setup work. An administrator token removes authorization errors from the demo. It also turns any tool call, script, or dependency running in that job into a potential production actor.
...
async function deleteApprovedPreview(input: {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (
      !approval ||
      approval.action !== "delete-preview-environment" ||
    ...
    return production.deletePreviewEnvironment(approval.resourceId)
  })
}
```

This illustrative action can delete only a preview environment selected by the trusted server. It cannot accept an arbitrary operation or resource type. A real service should compute `requestHash`, authorize the approver, verify the resource classification, and avoid holding a database transaction open across a slow provider call. It can atomically claim the approval first, then record the provider result in a separate idempotent execution record.
