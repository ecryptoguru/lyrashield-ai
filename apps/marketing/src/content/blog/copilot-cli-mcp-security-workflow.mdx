...
Use Copilot CLI's documented separator form for local stdio:

```bash
copilot mcp add lyrashield -- npx -y @lyrashield/mcp@0.2.8
```

For remote HTTP, options precede the server name and URL:

```bash
copilot mcp add --transport http --header "Authorization: Bearer YOUR-TOKEN" lyrashield-remote https://app.lyrashieldai.com/api/mcp
```

The user file is `~/.copilot/mcp-config.json`; project files are `.mcp.json` or `.github/mcp.json`. Never put a raw header in a committed project file. See the [official Copilot CLI MCP guide](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-mcp-servers) and [LyraShield guide](/docs/integrations/copilot-cli).
...
Keep installation, authentication and security results as separate acceptance checks. `copilot mcp list` establishes that Copilot loaded the server definition. A read-only LyraShield target query establishes authenticated tool use. A recorded scan establishes only its declared target, revision and coverage. The runner-local Action establishes its own diff result and cannot be substituted for the hosted receipt. Record all four when reviewing an exact release.

Recheck the server after upgrading Copilot CLI or the LyraShield package. Confirm the config location, transport, allowed tools and credential source, then repeat one read-only query before enabling any approval-gated mutation. If access is revoked or the workspace role changes, the next call must fail rather than reuse a previous successful response. This bounded sequence gives reviewers useful evidence without claiming broader desktop acceptance than was exercised.
