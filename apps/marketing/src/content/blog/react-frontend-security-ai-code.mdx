...
_React 19.2 common DOM and Server Function guidance was reviewed against official documentation on July 17, 2026. The server examples are architectural patterns and must be adapted to the backend framework actually in use._
...
## The risky pattern: a hidden control as authorization
...
## Enforce the action on the server

The server should derive identity from a verified session, load current authorization state, validate the target identifier, and perform the action under that constraint.

```ts
export async function deleteUser(request: Request, targetId: string) {
  const actor = await requireVerifiedUser(request)
  const membership = await loadMembership(actor.id)

  if (!membership || membership.role !== "ADMIN") {
    return new Response("Not found", { status: 404 })
  }

  if (targetId === actor.id || (await isLastOwner(targetId))) {
...
This example adds two common edge cases, but it is not complete. Real deletion may require recent authentication, audit evidence, invitation cleanup, object ownership transfer, notification, concurrency protection, and privacy retention rules. The key is that no browser flag proves the role.
...
## Test the server without trusting the interface
...
## State what the review establishes
