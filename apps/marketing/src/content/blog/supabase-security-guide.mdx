---
title: "Supabase Security: RLS, Service Keys, and OAuth Tokens"
description: "Review Supabase RLS, service-role bypasses, and OAuth token storage. Test signed-out and cross-account access before releasing your app."
pubDate: 2026-07-17
...
## Store third-party OAuth tokens outside browser-readable rows

[Supabase's provider-token guidance](https://supabase.com/docs/guides/auth/social-login#provider-tokens) distinguishes provider credentials from a Supabase session: Auth does not store those tokens in your database or refresh them for you. If your backend needs continuing provider access, send the tokens only to a trusted server you control.

Keep connection metadata separate from credential storage. A user-visible row can contain the provider, connection status, and owner; token ciphertext belongs in a private schema or secret store with no browser read grant. Bind ownership to the verified session, restrict the backend role, and keep encryption keys separate. RLS alone does not constrain a service-role client.

Test that Account B cannot invoke Account A's connection and that even Account A's normal API response contains no token. Review refresh, revocation, logs, and backups separately. This is a design checklist, not a complete credential-storage implementation.
