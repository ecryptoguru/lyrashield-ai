...
Each route passes an explicit policy with `credentials: true` only when that browser flow needs ambient credentials. Noncredentialed routes omit `Access-Control-Allow-Credentials`; they do not send it with a false value. If the response already varies on other headers, `appendVary` preserves those tokens and adds `Origin` once. This tells caches that responses for different origins are not interchangeable.
