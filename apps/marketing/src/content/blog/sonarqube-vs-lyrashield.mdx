...
faq:
  ...
  - q: "Is LyraShield a code quality tool?"
    a: "No. LyraShield focuses on security release assurance for AI-built apps: an agentic pentest, SCA, secrets, evidence states, and reviewed fix proposals. It does not replace SonarQube for code quality, maintainability, duplication, or architectural analysis."
...
Teams choose LyraShield when a quality gate is not the same as a release decision. The agentic pentest attempts to verify findings against an authorized running target and retains each result’s evidence state, the immutable evidence record gives reviewers and auditors something concrete, and reviewed fix proposals leave responsibility for code changes with the team. For apps where the code came from a coding agent and the team needs to sign off before production, that loop is the difference between a clean quality gate and a defensible release decision.
