...
The browser-local [secret exposure scanner](/tools/secret-exposure-scanner) checks up to 20 selected text files, skips non-text files and files over 1 MB, and redacts matches before display. It does not inspect Git history, deployed bundles, logs, cloud stores, or already leaked credentials.

The passive public [LyraShield Lite Check](/scan) covers a separate public-surface boundary. It cannot establish that a repository or build pipeline contains no secrets.
