...
faq:
  ...
  - q: "Which tool should I pick for an AI-built app?"
    a: "Use Semgrep when you want fast, customizable pattern scanning and you will triage findings yourself. Use LyraShield when you need an agentic pentest to prove exploitability, an immutable evidence record and reviewed fix proposals before a release decision."
---

Semgrep is a fast, open-source static analysis tool that finds insecure code patterns across more than 30 languages, with a commercial AppSec Platform that adds SCA, secrets and cross-file taint analysis. LyraShield AI is release assurance for AI-built apps: it pairs an agentic pentest with SCA and secrets, separates detection from proof and produces an immutable evidence record with reviewed fix proposals. Semgrep excels at pattern detection; LyraShield closes the proof and release-decision gap.
...
LyraShield also runs inside AI coding agents through the [Model Context Protocol](https://modelcontextprotocol.io/), so security checks happen at the point where an agent generates code, not only in a later CI scan. This is a different integration point than Semgrep's CLI and IDE plugins, which are designed for developer-initiated scans. Fix proposals are recorded for review; repository controls govern whether a developer merges any resulting change. The full assurance record is immutable and can be shared for compliance or handoff.
...
The reviewable fix proposal model reflects a specific bet about AI-built apps. When an agent writes code, an unattended auto-fix can compound the problem: the agent that introduced a flaw is also trusted to rewrite the fix and a merged patch that is subtly wrong can be harder to catch than the original finding. LyraShield records a proposal for review and a fresh retest can establish whether the finding persists. The repository governs any code change. That adds a checkpoint, but it is the checkpoint that makes the resulting evidence record trustworthy enough to base a release decision on.
...
Teams choose LyraShield when a scan report is not enough to ship. The agentic pentest attempts to validate exploitability against an authorized running target and retains each result’s evidence state, the immutable evidence record gives reviewers and auditors something concrete to sign off on and reviewed fix proposals leave repository merge controls with the team. For apps where the code was generated by a coding agent and the team needs to make a go or no-go decision, that loop turns a backlog of findings into a defensible release decision.
