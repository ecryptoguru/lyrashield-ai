...
The corrected pattern depends on the provider. Verify the provider's signature over the exact raw body, use constant-time comparison or the provider SDK, enforce any timestamp window, and deduplicate the event ID before changing state.
...
export default async function webhook(req: Request) {
  ...
  await applyEventAtomically(event)
...
Signature verification is only the first webhook decision. Providers retry when a handler times out or returns an error. Store a provider event ID before applying a non-idempotent change, and make duplicate delivery safe. If processing is slow, authenticate and persist the event, return the provider's expected success response, then handle it through a bounded queue.
