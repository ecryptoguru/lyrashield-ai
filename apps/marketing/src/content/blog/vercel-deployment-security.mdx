---
title: "Vercel Security: Preview Protection and Environment Variables"
description: "Secure Vercel previews, keep private environment variables out of browser bundles, and test deployment protection without weakening app authorization."
pubDate: 2026-07-17
...
### Public environment-variable prefixes expose browser values

A framework's public prefix marks values intended for client code. A provider secret must stay in server-only code and must not use that prefix. Removing the prefix alone is insufficient if a route, build script, or log still returns the value. Review the framework's variable rules and the generated JavaScript, not only the Vercel dashboard.

If a credential reached a browser bundle, rotate it, update server-only configuration, and redeploy. Variable changes do not repair older builds; retire or restrict affected deployments too. Use the [frontend API key guide](/blog/api-keys-frontend-ai-apps) for containment. Never place a live secret in a screenshot or test report to prove exposure.
