...
faq:
  ...
  - q: "Should automation bypass use a header or query parameter?"
    a: "Vercel recommends the bypass header for test tooling. Some webhooks require the query form, which is more likely to appear in histories, logs, referrers, or screenshots. Scope distribution and rotate on exposure."
---

**Direct answer:** Treat every Vercel preview as a live environment. Enable the appropriate Deployment Protection scope, isolate preview credentials and data from production, use branch-specific variables where needed, store automation bypass secrets outside URLs when the caller supports headers, validate callback domains, keep application authorization active after deployment access, and retire share links, exceptions, secrets, and stale deployments when review ends.

_Vercel Deployment Protection, environment, share-link, and automation-bypass behavior was reviewed against official documentation on July 17, 2026. Availability and protection scope can change, so verify the current project controls before release._
...
6. shareable links, protection exceptions, automation bypass consumers, and OPTIONS allowlists;
...
Protection Bypass for Automation uses a project secret. Vercel recommends the `x-vercel-protection-bypass` header for test tools. Store it in the CI secret store, mask it in logs, limit which jobs can read it, and rotate it if a fork, artifact, or debug output exposes it.
...
3. Confirm the approved reviewer and CI job can enter using their separate methods.
...
7. Revoke the share link or bypass, remove exceptions and callbacks, delete temporary data, and confirm the old path no longer works.
