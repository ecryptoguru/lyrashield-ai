...
- GitHub Action that is diff-aware and SARIF-emitting. LyraShield's action, agentic pentest + SCA + secrets, produces SARIF for GitHub code scanning, then evidence, fix, retest, report. That flow is on the roadmap; the platform is live in open beta with open registration.
...
Integration is deliberately small: stdio via `npx -y @lyrashield/mcp` and remote Streamable HTTP at `https://app.lyrashieldai.com/api/mcp` with `lsk_` Bearer per [integrations hub](/docs/integrations). The full loop, fix and retest, is on the roadmap (Sprints 0-2 shipped, scan engine in progress). The platform is live in open beta with open registration, with hardened MCP config and early architecture review for teams shipping AI-built code via Hermes alongside [pi-coding-agent-app-security-checklist](/blog/pi-coding-agent-app-security-checklist) and [openclaw-app-security-checklist](/blog/openclaw-app-security-checklist).
