...
faq:
  - q: "How do I add LyraShield MCP server to Amp?"
    a: "Run amp mcp add lyrashield -- npx -y @lyrashield/mcp@0.2.8. Amp stores always-available local definitions in amp.mcpServers."
...
You can wire Amp directly to LyraShield AI via MCP to get target aware verification inside your agent loop. In this guide you will add the local stdio server using pinned npx -y @lyrashield/mcp@0.2.8 and the remote server at https://app.lyrashieldai.com/api/mcp with bearer auth, confirm tool listing, run a scoped scan for findings with recorded evidence states, and enforce a diff aware GitHub Action gate that uploads SARIF for code scanning.
...
This pins stdio to `@lyrashield/mcp@0.2.8`. The canonical reference lives in [Amp integration guide](/docs/integrations/amp). Workspace definitions require Amp approval before they run; user settings and remote definitions have separate trust and authentication paths.
