...
Before launching an Antigravity built app, review what data entered the agent, protect its config and rules files, verify the resolved dependencies, test authorization and tenant boundaries, and collect independent launch evidence. Antigravity uses `~/.gemini/config/mcp_config.json` globally and `.agents/mcp_config.json` per workspace. The editor accelerates implementation but does not verify the runtime security of what it helped create.
