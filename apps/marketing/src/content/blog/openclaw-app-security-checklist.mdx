...
### Q: What about OpenClaw's `codex` projection controls?
`mcp.servers.<name>.codex` with `agents: ["main"]` and `defaultToolsApprovalMode: approve|auto|prompt` scopes that MCP server to listed OpenClaw agent ids for Codex app-server threads only. OpenClaw strips `codex` block before passing native `mcp_servers` to Codex. Does not affect ACP or other adapters.
...
OpenClaw gives you local-first gateway power: one control plane for 29 channels, client-and-server MCP, and a verified registry with `doctor`/`probe`. LyraShield is designed to be the narrow verifier on that gateway: `Target → Scan → Verified Finding → Fix PR → Retest → Report` with SCA and secrets feeding SARIF, not another general-purpose agent.

The integration surface matches OpenClaw primitives exactly: local `npx -y @lyrashield/mcp` for developer machines and remote `https://app.lyrashieldai.com/api/mcp` with `lsk_` Bearer for centralized revocation, both behind `toolFilter` and `requestTimeoutMs`. The combination is about channel-to-release integrity, inputs from 29 channels go through least-privilege MCP, out through diff-aware CI, with Fix PR → Retest intended to prevent drift. Scan engine MVP, Fix-PR, and billing are not live; current value is waitlist access, integration YAML validated by `openclaw mcp doctor --probe`, and participation in hardening the gateway-to-CI path.
...
Join the waitlist for OpenClaw config templates, `toolFilter` presets, and early access to the verification loop: https://lyrashieldai.com
