...
faq:
  ...
  - q: "How do I add LyraShield to OpenClaw?"
    a: "Use openclaw mcp add lyrashield --command npx --arg -y --arg @lyrashield/mcp@0.2.7, or add the remote URL with transport streamable-http and auth oauth."
...
- Prefer `streamable-http` for bidirectional streaming with remote verifiers. `sse` is legacy compat. Use `stdio` for local-first verifiers like `npx -y @lyrashield/mcp@0.2.7` to keep secrets on host.
