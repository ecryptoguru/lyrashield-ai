...
faq:
  ...
  - q: "How do I add LyraShield to OpenClaw?"
    a: "Use openclaw mcp add lyrashield --command npx --arg -y --arg @lyrashield/mcp@0.2.8 or add the remote URL with transport streamable-http and auth oauth."
...
Record the exact OpenClaw version, enabled channels, MCP server definitions, tool filters and authentication method with each acceptance run. A successful static config check establishes syntax only; use `doctor --probe` for connection evidence and retain the resulting scope separately from the application security assessment.
...
Run `npx lyrashield init --agent openclaw` to print the guided setup. Apply it through OpenClaw's `mcp add` commands or Control UI; LyraShield does not edit the client-owned registry automatically.
...
- Use OAuth for the LyraShield remote. `openclaw mcp login <name>` performs the MCP OAuth flow and stores credentials under OpenClaw state.
...
- Prefer `streamable-http` for bidirectional streaming with remote verifiers. `sse` is legacy compat. Use `stdio` for local-first verifiers like `npx -y @lyrashield/mcp@0.2.8` to keep secrets on host.
...
OpenClaw gateway configuration should be reviewed as executable access policy. The [OWASP Top 10](https://owasp.org/www-project-top-ten/) supplies a baseline for authorization and injection review, [CWE-798](https://cwe.mitre.org/data/definitions/798.html) covers hardcoded credential exposure and the [Model Context Protocol specification](https://modelcontextprotocol.io/specification/) defines MCP transport and tool contracts. These references are not a security guarantee. They help reviewers inspect which channels and tools can act, where credentials resolve and whether a release record binds the configuration revision to its stated limits. Repeat this review after changing a gateway, channel, skill, MCP server or approval boundary.

Keep that record scoped to the gateway revision reviewed.

OpenClaw gives you local-first gateway power: one control plane for 29 channels, client-and-server MCP and a verified registry with `doctor`/`probe`. LyraShield is the narrow verifier on that gateway: release assurance for AI-built apps with `target, review, evidence, fix, retest, report` and SCA and secrets feeding SARIF, not another general-purpose agent. LyraShield separates detection from proof: recorded scans contribute to an assurance record and fix proposals are approval-gated, PR execution is blocked until a server-generated patch is bound to the exact approval.

The integration surface matches OpenClaw primitives: local `npx -y @lyrashield/mcp@0.2.8` for developer machines and remote `https://app.lyrashieldai.com/api/mcp` with revocable authentication, both behind `toolFilter` and `requestTimeoutMs`. The combination is about channel-to-release integrity: least-privilege MCP inspection, an advisory diff-aware CI gate and a separate recorded retest. The platform is live in open beta; scans, findings and evidence, approval-gated fix proposals, retests, reports and billing are implemented. Production availability remains bounded by release gates and automatic server-generated Fix PR execution is not enabled.
...
Create an account at [lyrashieldai.com](https://app.lyrashieldai.com/sign-up) for OpenClaw config templates, `toolFilter` presets and access to the verification loop.
