...
Set `sandbox.failIfUnavailable` when sandboxing is a release requirement. Anthropic documents that sandbox startup can otherwise warn and continue without the sandbox on an unsupported or incomplete system. A visible warning is useful during exploration. It is not a reliable CI gate.
