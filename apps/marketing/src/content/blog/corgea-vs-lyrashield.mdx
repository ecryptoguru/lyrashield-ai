...
faq:
  ...
  - q: "Is Corgea pricing public?"
    a: "Corgea publishes plan pricing on its site (a free tier and paid per-developer plans as of September 2026). LyraShield is in open beta with open registration at lyrashieldai.com. Check the Corgea pricing page for current figures before comparing costs."
---

Corgea is an AI-powered security platform founded in 2023 that automatically finds, triages and fixes insecure code, with auto-discovery that adapts to your codebase and AI pentesting that validates exploitability. LyraShield AI is release assurance for AI-built apps: it pairs an agentic pentest with SCA and secrets, separates detection from proof and produces an immutable evidence record with reviewed fix proposals. Corgea excels at adaptive triage and auto-fix; LyraShield closes the proof and release-decision gap.
...
Use LyraShield AI when the specific need is a release decision for an AI-built app, backed by proof, an immutable record and reviewed fix proposals with a mandatory retest. The tools can complement each other depending on whether the priority is backlog reduction or release sign-off.
