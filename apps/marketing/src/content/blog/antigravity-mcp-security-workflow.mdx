...
To connect Antigravity to LyraShield, run `npx lyrashield install antigravity` so the CLI writes the MCP server entry to its dedicated config, then run `npx lyrashield doctor` to verify it. Antigravity uses `~/.gemini/config/mcp_config.json` globally and `.agents/mcp_config.json` per workspace.
