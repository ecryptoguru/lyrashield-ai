...
Refreshing the evidence rotation set updates the root-only environment file; restart the service before any new evidence is written. After restart, run the exact-ID retained-evidence verifier against at least one artifact under every retained key reference (the former deployment runbook was removed 2026-09-09; its verifier procedure lives in git history). The verifier returns only a fixed success marker and never logs content, private URIs, or keys.
