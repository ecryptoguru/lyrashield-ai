...
Digest pinning prevents silent updates; it does not freeze the worker. After the application repository's main deployment verifies a SHA-only worker image, its exact digest, and its OCI labels, the production workflow stops admission, proves the database and both queues are idle, pre-pulls the digest, updates only `LYRASHIELD_WORKER_IMAGE`, retains the previous configuration, restarts the service, verifies provenance and readiness, then resumes admission. The launcher also pulls an immutable image when it is absent locally, so an empty VM cache cannot break a reviewed promotion.
...
Pin refresh does not inspect or interrupt the active job. The firewall union and in-place hosts update keep existing and new endpoint paths valid throughout rotation, so BullMQ clients, readiness, and paid work remain untouched. A failed refresh leaves the old committed pins plus the union for a later timer retry.

Do not place secrets in the runtime configuration. `refresh-secrets.sh` owns the exact Key Vault-to-environment mapping and fails closed when a required secret is absent or empty. The Key Vault must contain both `worker-database-url` (the RLS-restricted runtime role) and `worker-database-system-url` (the privileged ownership-check role), the BullMQ TLS endpoint as `worker-redis-url`, both authenticated egress-proxy secrets, and, after migration, `worker-evidence-kek-config-ref`. That selector has the exact form `vN/<12-character-keyring-digest>`; refresh derives immutable `worker-evidence-kek-vN` and `worker-evidence-kek-keyring-<digest>` names. When the selector is absent, refresh uses the legacy fallbacks below; when it is present but malformed, refresh fails closed. `run-worker.sh` initializes the host-visible shared run root through a networkless one-shot container, then starts the application as the image's non-root user.

Refreshing the evidence rotation set updates the root-only environment file; restart the service before any new evidence is written. After restart, run the exact-ID retained-evidence verifier documented in `docs/deployment/PRODUCTION_DEPLOYMENT.md` against at least one artifact under every retained key reference. The verifier returns only a fixed success marker and never logs content, private URIs, or keys.

Migration is backward compatible until the new selector is provisioned. Refresh tries `worker-evidence-kek-config-ref`, then the earlier `worker-evidence-kek-active-ref` plus versioned secrets, then the original fixed `worker-evidence-kek` as v1 with `{}` keyring. Fallback happens only when a newer selector is absent; a present malformed selector fails closed. Pre-provision the selector's immutable active and keyring targets before setting the selector, and never use the empty legacy v1 fallback after v2 evidence may exist.
