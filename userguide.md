...
`init`/`install <agent>` choose an install strategy based on the agent. `packages/agent-registry` contains 30 entries and resolves them into 26 preferred client surfaces. CLI `0.2.9`, MCP `0.2.7` and Agent Plugin `0.1.26` require Node 24 or newer.
