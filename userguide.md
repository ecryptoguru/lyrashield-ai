...
- Use **Sign in** with your registered email and password.
...
The public marketing site, Lite Check, browser-local tools, methodology, and content are live. The authenticated dashboard and full BullMQ/engine worker remain separate deployment surfaces and are not yet a public production capability.

An operator must provision the authenticated application origin, TLS Redis queue, private evidence storage, sandbox-capable worker compute, authorized models, monitoring/recovery, and transport-level egress controls before offering full scans broadly.
...
- Confirm the account has completed email verification when required.
- Clear the site session and sign in again.
- Use password reset if the password is uncertain.
