...
- The PR #108 release candidate passes **853 core tests in 94 files**, **79 marketing tests in 12 files**, **16 motion tests**, and **2 Chromium E2E tests**, plus lint, typecheck, production build, formatting, Prisma client generation, migration drift/application across **21 migrations**, SCA/secret scanning, and `git diff --check`. The last merged-`main` count remains a historical boundary until the PR merges.
...
- **Azure AI / GPT 5.6 mode routing and spend caps are implemented**: Safe/Quick/Standard scans select `LYRASHIELD_LUNA_LLM` at `medium` reasoning; Deep/Custom select `LYRASHIELD_TERRA_LLM` at `high`. `LYRASHIELD_LLM` remains the backward-compatible fallback. Default engine caps are $1.20, $1.20, $3.20, $15, and $15 respectively; a positive workspace `Policy.maxBudgetUsd` overrides the mode default. The chosen model/reasoning and budget cap are retained as scan events. An engine-reported overage now terminates as `STOPPED_BUDGET` and persisted billable cents are clamped to the approved cap. When the engine supplies complete standard-context token counters but no cost, the worker calculates cost from the versioned official OpenAI GPT-5.6 Sol/Terra/Luna rate card and records that provenance; ambiguous long-context aggregates fail closed. Azure credentials and endpoints remain shared through `AZURE_AI_*`, `AZURE_OPENAI_*`, or generic `LLM_*` variables.
- **Assurance controls and cost visibility (PR #108 release candidate):** the dashboard exposes Release Check, Code Review, and Deep Security Review plus Weekly Monitor schedules; URL/API targets display $0 AI-model cost because they skip the external engine. Scan details retain engine-reported and capped billable cost, request/token counters, immutable manifests, per-control receipts, and the Vibe Security 50 ledger. The complete end-user workflow and every current option are documented in `userguide.md`.
...
- **Model cost routing**: keep model selection in `resolveEngineProfile()`, spend defaults in `resolveScanBudgetUsd()`, and official GPT-5.6 rates in the versioned `apps/worker/src/engine/gpt56-pricing.ts` rate card. Do not bypass the positive policy-budget validation, remove the fallback model, silently guess ambiguous long-context pricing, or claim that mode routing is a within-scan Luna→Terra cascade.
...
- `userguide.md` — complete end-user workflows, options, result language, costs, roles, and limitations
