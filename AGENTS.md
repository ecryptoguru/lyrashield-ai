...
### Local branch truth — authority blog release candidate (not merged or deployed)

- The current branch maps exactly 100 locally approved authority-blog articles and a 36-image source-artwork library. All entries are `draft: false` for local release testing; this is not production, sitemap, RSS, or deployment truth.
...
- Remaining gates are complete local validation, Worker-backed browser and performance QA, final local approval, a focused PR, green CI, guarded marketing deployment, and live canonical/sitemap/RSS/image/schema verification. Until those pass, do not record this program as merged or live and do not make Cloudflare changes for it.
