...
- Product `main` includes the billing-staging hardening from PR #426 and Redis/egress efficiency work from PR #432 at `3629b442`. Neither change is deployed. Release run `32755678337` built and pushed earlier images, then failed closed before Azure login, migrations, revision creation, or runtime mutation because the protected Razorpay credential is Test Mode while production requires Live Mode. Production remains healthy on `8347fda9`; all Cloud and Local purchase admissions remain `off`.
...
- The merged worker Redis/egress efficiency code is not deployed. It adds slower idle BullMQ polling, single-key Lua heartbeat/readiness operations, DB-first reconciliation, proxy-only CISA enrichment, and drain-safe pin rotation; live Redis command metrics and staged rollout evidence remain required.
