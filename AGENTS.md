...
- Current `main` passes **858 core tests in 94 files**, **79 marketing tests in 12 files**, **16 motion tests**, and **2 Chromium E2E tests**, plus lint, typecheck, production build, formatting, Prisma client generation, migration drift/application across **21 migrations**, SCA/secret scanning, and `git diff --check`.
...
1. Keep `LYRASHIELD_LUNA_LLM`/`LYRASHIELD_TERRA_LLM` and the tested `LYRASHIELD_LLM` fallback authorized for the target environment. Local Safe-mode routing has been exercised; Deep/Terra still needs its own approved run. Engine PR #6 adds pre-request history compaction at a 240k-token estimate toward a 180k target plus a 200 kB dedupe-prompt bound; its CI is green but it is not engine-`main` truth until the required independent approval and merge occur.
