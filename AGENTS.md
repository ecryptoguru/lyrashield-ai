...
10. Complete the exact-two production platform-admin preflight, and only apply roles after both named users personally verify email and enroll TOTP; then capture fresh-session MFA browser proof. PR #413 fixed Prisma generation. Retries `32729923445` and `32730207675` then proved the provisioning script was importing unrelated application env validation before account reads; the follow-up isolates its Prisma client to `DATABASE_SYSTEM_URL`.
