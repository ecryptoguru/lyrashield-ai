...
- The current billing-staging candidate is based on `main` at `d7f38001` after PR #431. Release run `32755678337` built and pushed earlier images, then failed closed before Azure login, migrations, revision creation, or runtime mutation because the protected Razorpay credential is Test Mode while production requires Live Mode. Neither those images nor this candidate is deployed. Production remains healthy on `8347fda9`; all Cloud and Local purchase admissions remain `off`.
