...
- **Current production Standard acceptance (2026-08-21):** scan `cmt35aj1s000001hck9fmguzk` against `ecryptoguru/OnboardingAI2@1689f3607d68764e09769535df8e368c4d5ad2fe` completed in 11m 42s on Luna/medium only: 184 requests, 8,227,004 input tokens (6,066,725 cached), 30,844 output tokens, and $0.597148 provider/billed cost under the $3.20 cap. It retained 24 findings, 24 candidates, 24 verification receipts, and 56 coverage receipts; none of the findings was independently verified, and the AI App Security layer recorded its 200-file bound. The Standard usage debit was 12 agent-minutes. This is a successful bounded Standard run, not a universal security or full-coverage claim.
...
- **Production recovery and egress (2026-08-21):** PR #377 added Prisma client generation to the isolated restore drill; a manual run completed encrypted backup, restore, schema/RLS/audit verification, and application startup. PR #376 added the digest-pinned egress-proxy image and Azure deployment hook. PRs #380–#381 pinned engine `dd588c379ae6614e0914b8adb41d94f0c1e86c26`, enforced the authenticated proxy for production URL scans, moved BullMQ to managed Upstash TLS, and persisted scanner readiness. PR #382 made DNS-pin refresh restart-safe for active scans. The verified worker digest `sha256:03f7a6b2d14bda224b14a9641a9acaeafa54c400f1b13fae3f002cdbb59e4f44` runs product revision `8f5257a96f5d8cbd299f5152215ac401ed16755c`; authenticated Redis returned `PONG`, the queue was empty before acceptance, `AllowRedis6379` is deleted, and the legacy `lyrashield-redis` container remains stopped/restart-disabled for rollback. The live worker proof denied a direct arbitrary public fetch, allowed the authenticated proxy fetch, and returned `ssrf_blocked` for loopback. The refresh timer stayed active during the live Standard scan without restarting the worker. A literal private-network Redis endpoint remains an enterprise option, not an open-beta prerequisite.
...
1. Current Standard/Luna acceptance completed (2026-08-21). Deep/Terra still needs its own approved run.
...
- **Email verification:** `LYRASHIELD_REQUIRE_EMAIL_VERIFICATION` is validated at boot in `packages/config/src/env.ts`. The live Container App has the existing Brevo secret reference and verification was enabled in revision `lyrashield-app--0000149` on 2026-08-21; readiness remained healthy after the rollout.
