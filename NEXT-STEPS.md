...
- Sprint 5+: SCA + secrets scanning, AI-builder-aware URL scan, launch-readiness gate, shareable report/badge, MCP server + prompt-injection defense, GitHub Action diff-gate, billing, agent/MCP layer. The scan queue/worker/engine **is now built** (Sprint 4) — BullMQ queue, preflight, engine runner, output parser, finding persister are all implemented and tested. **Fix proposals, retests, reports, notifications, schedules, and plain-language findings are now built** (Batch 4) — see `codebase.md` §22.
...
| 7 | Fix Proposals | DONE (Batch 4) | codebase.md §22 |
