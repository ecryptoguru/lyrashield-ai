...
- **Automation:** Engine commit `909493f` adds a Monday 03:23 UTC/manual, PR-only sync workflow. It uses the ancestry gate, exits `20` before rebasing rewritten upstream history, verifies before creating `automation/upstream-<short-sha>`, and has no auto-merge, force-push, or conflict resolver. It is published in [engine PR #1](https://github.com/ecryptoguru/lyrashield-engine/pull/1); it will be dispatchable after merge.
...
- **Release blockers, do not paper over:** The worker Docker gate now builds by compiling only `@lyrashield/web`, leaving the unrelated uncommitted marketing workspace out of the image build. Local image ID `sha256:71d6c104f5d11e30d8f8ee63cef8aacb1819b5ec8a4c3d1987d7fd3dcaddc4e6` passed `lyrashield --version` (`1.0.4.post1`); explicitly empty model variables caused an early exit `1` with `STRIX_LLM` guidance and no sandbox pull/download. This is not a sandbox run or controlled scan. The inspected production image is `ghcr.io/usestrix/strix-sandbox@sha256:478e0b37ec83b2ba8c6e159593cb46d5dc9b624a45d6a9bb606851b83058d284`. `LYRASHIELD_LLM` and `LLM_API_KEY` are absent for authorized use, so no controlled scan may use a substitute target; missing authorized LLM configuration remains the controlled-scan blocker.
