...
- **Launch assurance wave (WP1–WP7, merged 2026-09-02).** The Launch Gate (named versioned readiness standard, READY/NOT_READY/INSUFFICIENT_EVIDENCE verdicts, append-only per target), the shareable signed Launch Readiness Report with public verification, the AI-Built Failure Taxonomy (public, citable, control-traced), WebMCP Assurance at 14 controls, the approval-gated fix-PR pipeline (deterministic patch generation from engine structured fixes, plan-tiered scope, real retest loop-closure on merge), and the CLI `gate --verdict` surface with a stable 0/1/2 exit-code contract.
