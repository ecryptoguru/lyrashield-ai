...
> **2026-07-04 addendum (internal):**
> - **Product name migrating LyraShield → LyraSec AI; repo is now `ecryptoguru/lyrasec-ai`.** Final public name + domain remain founder decisions #1–#3 (trademark clearance open) — build domain-agnostic, do not commit to a public URL/handle yet.
> - **v1 coverage FINAL (decision #15):** agentic pentest **+ SCA (dependency/vulnerable-package) + secrets scanning** + a first-class GitHub Action / reusable workflow with a diff-aware PR gate + **SARIF** output. This matters for messaging: v1 covers the deterministic layers (deps, secrets) buyers check first, *and* the deployed-app/backend-config layer where documented vibe-coding breaches actually happen — not agentic-pentest-only. Still honest-positioning bound: SCA/secrets are table-stakes matched by many; lead with the *combination + loop + review package*, never "only we." IaC/container + reachability stay Phase 2.
