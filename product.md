...
- The `agent-registry` package that renders 30 entries into 26 preferred client surfaces in JSON/JSONC/TOML/YAML. Preferred paths comprise 13 config-file clients, Amp through its vendor CLI, seven guided-manual clients, and five Agent Plugin installs; three legacy config-file alternatives remain addressable for plugin-preferred clients.
- Agent Plugins v1.0.0 support: the portable `@lyrashield/agent-plugin` package with generated client shims for Claude Code, Cursor, OpenAI Codex, and Kiro; GitHub Copilot uses the portable root manifest. VS Code deliberately stays on its verified config-file path.
...
- **Cloud billing (Sprint 10 + WP1 repricing, merged).** Polar + Razorpay dual-gateway with 14-day trial, plan-based entitlements (TRIAL/STARTER/PRO/LAUNCH_ASSURANCE/ENTERPRISE — two product lines: Scan and Launch Assurance), usage metering (agent-minutes with Deep 3x multiplier), minute packs, Launch-Assurance-only overage with spend limit, grace period, and geo-routing (India→Razorpay/INR, global→Polar/USD). Failed scans are never billed; cancelled scans bill elapsed time only.
- **Launch assurance wave (WP1–WP7, merged 2026-09-02).** The Launch Gate (named versioned readiness standard, READY/NOT_READY/INSUFFICIENT_EVIDENCE verdicts, append-only per target), the shareable signed Launch Readiness Report with public verification, the AI-Built Failure Taxonomy (public, citable, control-traced), WebMCP Assurance at 12 controls, the approval-gated fix-PR pipeline (deterministic patch generation from engine structured fixes, plan-tiered scope, real retest loop-closure on merge), and the CLI `gate --verdict` surface with a stable 0/1/2 exit-code contract.
- **LyraShield Local/Desktop (Sprint 10, merged 2026-08).** Tauri v2 BYOK desktop app with one-time 1-year license, ed25519 signed licenses, offline grace, perpetual fallback, ChatGPT OAuth + Azure OpenAI BYOK, optional cloud sync, and macOS/Windows build pipelines prepared for code signing. Production signing, notarization, and distribution proof remains outstanding.
...
- Production Standard scans now have a promoted sandbox-capable worker, managed BullMQ-compatible TLS Redis, an authenticated application origin, backup/restore and private-evidence proofs, actionable alert acknowledgment, controlled queue recovery, and transport-level egress enforcement. Longer-window capacity evidence and separate approval/evidence remain required for each additional review profile.
...
Discovery foundation includes canonical URLs, structured data, sitemap/robots controls, dated `llms.txt`, machine-readable `agents.md`, explicit answer-engine crawler policy, integration guides, comparisons, research, and evidence-led editorial validation. Treat these as publishable inputs, not proof of Google indexing, answer-engine citation, rankings, traffic, or customer demand; retain external webmaster and citation receipts before making those claims.
